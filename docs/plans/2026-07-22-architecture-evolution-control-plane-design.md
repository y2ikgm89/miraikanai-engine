...
- 実行入口・公式根拠: [計画書の実行権限・準備状況](README.md)。exact external artifactは[Toolchain／Dependencies](../architecture/02-foundation/toolchain-dependencies.md)が所有する
- assurance boundary: `assurance_profile=development_bootstrap` と `production` は本設計の既存 closed profile であり、計画本文や recommendation が新しい簡略 trust profile を作らない
    ...
    {schema_id, schema_ref, schema_sha256,
     signature_slots[]:
       {signature_slot_id, branch_id, wrapper_field_path,
        ...
        authority_binding_time = construction | runtime_instance,
        issued_at_source_kind,
...
`AuthorityBindingSourceCatalogV1.members[]`はLocal Schema Catalogの全`signature_slots[]`との`{schema_id,signature_slot_id}` set equalityを正本とし、その順で重複なくschema hash、branch、purpose、`authority_binding_time`、tagged authority sourceを固定する。`authority_source_kind=trust_registry`だけはRoleとsubject／scope derivation四Fieldを全non-null、他三kindは全nullとする。`authority_binding_time=construction`はConstruction Authorizationが束縛した完成instanceからsubject／scopeを導出し、Task 0のrequired binding集合へ含める。`runtime_instance`は将来のTask／Approval／Receipt等、Construction時に存在しないinstanceのpayloadから発行時に導出し、Task 0で具体subject／assignment／Keyを捏造しない。ただし固定Role、purpose、導出Policy、schema slotはCatalogへ存在し、各instance検証時にcurrent Trust closureからexact Identity／Role／assignment／purpose Keyを解決する。offline Root slotは当該purposeのRoot Profile quorum、Recovery custodian slotはRecovery Policyのcustodian quorum、construction executor slotはConstruction Authorizationのexact subject／public keyへ解決し、これらを通常Trust Roleへ偽装しない。同purposeを持つsource／destination slot、Rotation old／new、Recovery三slot、Incidentのroot／recovery-custodian branchをslot／branch IDで区別し、inactive branchの署名混入とactive branchの欠落を拒否する。`catalog_id`は同Fieldを除くJCS hashから`urn:mirakan:authority-binding-source-catalog:sha256:<lowercase-hex>`として導出する。初回`AuthorityBindingSourceCatalogHeadV1`はprevious=null／sequence 1／`authorization_kind=offline_root_genesis`で完成Root Headを束縛し、Authorizationはその完成HeadとCatalogをref／hashで承認する。稼働後の変更はcurrent Trust closureとCatalog headをsource、destination Catalogとexact diffをdestinationとして`TrustRegistryChangeManifestV1`へ閉じ、fresh `TrustRegistryChangeApprovalV1`で承認し、Trust closure headとCatalog headを一つのexpected-parent CASで更新する。説明文の「少なくとも」列挙、未注釈schema introspection、Catalogだけの独立更新を正本にしない。

Catalog memberは`{schema_id,signature_slot_id}`のunsigned UTF-8 tuple順・重複なしとし、同一schema IDの複数slotを正規に許す一方、同一tupleを拒否する。Local Schema Catalog annotationからschema hash、branch、purpose、`authority_binding_time`、issued-at source／path、revocation context／path、authority source kindをbyte-exact複写し、一FieldでもCatalog側で変更しない。`authority_source_kind=trust_registry`だけがsubject／scope derivation ref／hashを完成`AuthorityBindingDerivationPolicyV1`へexact解決する。`exact_ref_field_v1`はsubject policyだけでinput path一件、fixed output／prefix=nullとし、そのcanonical subject refをbyte-exact pass-throughする。`catalog_fixed_ref_v1`はsubject policyだけでinput paths=[]、fixed output non-null、prefix=nullとする。`rfc8785-jcs-sha256-v1`はfixed output=null、non-empty field-path tupleをpath順のclosed objectへ投影してJCS SHA-256を計算し、subject policyならnon-null URN prefixへsuffix、scope policyならprefix=nullのlowercase hex 64を返す。input schema／path不存在、配列indexや表示名依存、unknown algorithm、nullability差、同入力で複数outputを拒否する。Task 0が生成または再利用確認するIdentity／Role／Assignment／purpose KeyとProvisioning Receiptの`required_authority_bindings[]`は、Catalogの`authority_binding_time=construction`かつtrust-registry slotだけから導出した`{schema_id,signature_slot_id,subject_ref,role_ref,signed_record_purpose,assignment_scope_sha256}`集合とset equalityにする。`runtime_instance` slot、offline Root／Recovery custodian／construction executor slotをこの集合へ混入しない。runtime instanceはwrapperごとに同じtupleへschema validationし、actual payloadからsubject／scopeを導出してcurrent Trust rowと照合する。

本Catalog群の`canonical field path`はRFC 6901 JSON Pointerだけである。root pointerはexact empty string `""`、token内の`~`と`/`はそれぞれ`~0`／`~1`へescapeし、relative JSON Pointer、JSONPath、wildcard、recursive descent、array indexを禁止する。各pathはCatalogが束縛するexact schema上の一Fieldまたはroot wrapperへ一意に解決し、missing、複数解決、optional branch外Field、表示名からの補完を拒否する。
...
六Registryの`registry_id`集合はschemaに示した六URNとset equality、初回revisionは1、以後はentry bytesにdiffがある場合だけexact `N+1`とし、同bytes revision bump／同revision別bytesを拒否する。Identityの`subject_ref`とRoleの`role_ref`はAuthority Catalogの完成derivation policyまたはcatalog memberのexact Role値からだけ生成し、表示名／email／配列順から推測しない。`assignment_id`は同Fieldを除くclosed assignment row JCS SHA-256から`urn:mirakan:authority-role-assignment:sha256:<lowercase-hex>`、Public Key `key_id`は§6.0.1のSPKI rule、`revocation_id`は同Fieldを除くclosed revocation row JCS SHA-256から`urn:mirakan:authority-revocation:sha256:<lowercase-hex>`として導出する。Governance Policy `policy_id`は`policy_schema_id`が所有するtagged `content_derived | fixed_logical` ID規則を検証し、前者はprojection hashを再計算、後者はschemaに列挙したexact logical IDへ一致させる。`policy.control-plane.governance-recovery.v1`と`policy.trust-registry-change-evidence.v1`はfixed logical branchである。同じpolicy IDのdestination bytes変更はRegistry revision exact `N+1`、Trust Change Manifestのmodified row、source／destination ref／hash、必要Evidence／Approvalを必須とし、old/new ref混在やin-place bytes置換を拒否する。

Catalogとのrequired-set検証と、六Registry全rowの認可検証を混同しない。まずcurrent完成Trust Headからprevious chainをgenesisまで逆走し、genesis `TrustProvisioningReceiptV1`が束縛した六Registry bytesを初期stateとする。その後の各sequenceについて、完成`TrustRegistryChangeManifestV1`または各closed Recovery／Remediation Authorizationが承認したrow-level `added | modified | removed`差分だけをkey順にforward replayし、source hash、destination hash、change kind、Approval、expected previousを毎段再検証する。これを`authorized replay state`と呼ぶ。current六Registryの各`{primary_key, completed_entry_sha256}`集合は、replay結果の同Registry-kind集合とそれぞれset equalityでなければならない。history gap、未承認row、別kindへの転用、sourceにないmodify／remove、destinationにないadd、同key別hashを拒否する。主keyは順にsubject_ref／role_ref／assignment_id／key_id／revocation_id／policy_idでuniqueである。

この全row replayとは別に、Catalog由来の下限をbinding timeごとに検証する。全`authority_source_kind=trust_registry` slot（`construction | runtime_instance`）のfixed `role_ref` unionはRole Registryのrequired subsetであり、全Role Registryとのset equalityではない。`authority_binding_time=construction` slotから導出したIdentity／Assignment／purpose Key集合だけをProvisioning Receiptのrequired binding集合とset equalityにする。`runtime_instance` signing Identity／Assignment／purpose Keyは0～N件で、row materialization時に少なくとも一つのruntime slot templateのunique fixed Role、singleton purpose、subject／scope derivation Policyおよび承認済みTrust Changeへexact逆解決する。署名直前に完成unsigned payloadからsubject／scopeを導出してcurrent rowと一致させ、その後に完成wrapper全体をschema／slot／signature検証する二段階とする。未使用だが承認済みのruntime signing rowは許可する一方、未使用templateから具体subjectを自動生成せず、construction不足へ数えない。

Catalog fixed Role以外のRole rowはexact二branchだけを許す。`qualification_only` branchは`permission_ids=[]`かつ`allowed_signed_record_purposes=[]`で、`independence_class`だけをquorum意味として持ち、承認済みTrust Changeの追加rowとauthorized replay stateへ存在する。Author／Owner／Code Owner／Domain Owner／Reviewer／Release Owner等はこのbranchであり、実行・署名権限を単独では与えない。使用時はAI Security／Approval正本の`OperationApprovalQuorumRuleV1.allowed_approver_role_refs[]`またはGrantの資格Roleにexact参照され、同human Identityのcurrent qualification AssignmentがProject descendant、Operation、typed scope、Risk／authority classをcoverする場合だけ資格になる。もう一つの`offline_or_recovery` branchは本節のclosed Root／Recovery schemaとAuthorization chainが列挙するRoleだけである。Catalogにないsigning purpose／permissionを持つRole、qualification Role用Key、Catalog fixed transport Roleを資格Roleとして暗黙流用することを拒否する。

Registry-kind別の追加受入predicateは次に固定する。Identityはconstruction binding、runtime signing binding、qualification Assignmentのsubject、closed offline／recovery authorityのいずれかへ到達する。Assignmentはconstruction binding、runtime signing binding、または`qualification_only` Roleへのtyped scope assignmentである。Public Keyはconstruction／runtime signing slotまたはclosed offline／recovery purposeへ逆解決し、qualification-only branchには存在しない。Revocationは§6のclosed target／reason／provenance branchを満たし、その追加差分を承認したManifest／Authorizationへ解決する。Governance Policy ConfigはLocal Schema Catalogのcompleted policy schema、completed policy object ref／hash、schema所有のID規則を満たし、そのgenesis projectionまたは承認済みTrust Changeへ解決する。Roleは前段のfixed／qualification／offline-recovery exact一branchである。したがってOperation Receipt Signer Policyのような正常なPolicy rowと、正当なRevocation rowを「fixed Role以外のextra」として拒否せず、同時に承認履歴にない任意Policy／Revocation／Identity／Role／Assignment／Keyを許可しない。

新しいTrusted Serviceとsingleton-purpose署名Keyを通常追加する場合、Trust Registry deltaはIdentity、Authority Role、generic Role Assignment、Authority Public Keyの四Registryへ各exact一row、RevocationとGovernance Policy Configへ0 rowである。signed purposeは別Registryを作らず、Role `allowed_signed_record_purposes[]`とKey `allowed_signed_record_purposes[]`の同じsingletonへ保存し、Assignmentはsubject／Role／Scope／validityだけを既存generic schemaで閉じる。Service Role Registry、Purpose Registry、Profile内Roleを追加しない。四rowの追加で四Registry revisionだけをexact `N+1`へ進め、変更のない二Registryは同revision／bytesを再利用する。六Registry、15-member Trust closure、required Authority Catalog binding、Trust Change Manifest／Approvalのset equalityを一つのatomic Trust／Catalog publicationで検証する。
...
ControlPlaneRebaselineCoreV1
  ...
  foundation_definition_closure_sha256
...
全objectはclosed、ref／隣接hashは同一完成bytes、配列はunsigned UTF-8 byte順・重複なしとする。Rebaseline Coreの`foundation_definition_closure_ref/hash`はA2 subject treeから再構成した[Executable Contractsの`FoundationDefinitionClosureV1`](../architecture/02-foundation/executable-contracts.md#51-foundation-definition-closure)へexact解決し、Contract Set、Owner Identity Registry、Named Algorithm Registryの三root、closure self-hash、Local Schema Catalogの対応schemaを再検証する。source closureは`source_control_plane_baseline_binding`から解決し、destination closureと別値ならその差をsame-definition rebaselineまたはdefinition migrationのsubjectへ明示する。さらにdestination Contract Setの全Capability `failure_modes[].fallback_id`をdestination Active Product Definitionの`FallbackRegistryV1`へexact一件解決し、Policy rejection tokenとProduct Diagnosticを区別する。`fallback.capability.unavailable`は`preserves_semantics=false`かつ、三Field`ProductDiagnosticRefV1`がProduct root内の`diagnostic.product.capability-unavailable` version 1 record／self-excluding hashへexact解決しなければならない。三rootの一部だけを更新すること、source closureのhashだけをdestinationへ流用すること、Foundation closureをTrust closureへ混入すること、Product fallback／Diagnostic refをFoundation hash preimageへ戻すことを拒否する。

ID projectionを型別に固定する。flat `ControlPlaneRebaselineCoreV1.rebaseline_core_id`とflat `CurrentControlPlaneBaselineBindingV1.binding_id`は同ID Fieldを除く完成object JCS SHA-256からそれぞれ`urn:mirakan:control-plane-rebaseline-core:sha256:`、`urn:mirakan:control-plane-baseline-binding:sha256:`として導出する。`ControlPlaneRebaselineApprovalPayloadV1.approval_id`と`ControlPlaneRebaselineTransactionPayloadV1.transaction_id`は同ID Fieldを除くpayload JCS SHA-256からそれぞれ`urn:mirakan:control-plane-rebaseline-approval:sha256:`、`urn:mirakan:control-plane-rebaseline-transaction:sha256:`として導出する。`ControlPlaneRecoveryRebaselineAuthorizationPayloadV1.authorization_id`も同Fieldを除くpayload JCS SHA-256から`urn:mirakan:control-plane-recovery-rebaseline-authorization:sha256:<lowercase-hex>`として導出する。Approvalはcurrent `role.control-plane-rebaseline-approver.r4`への独立human assignmentとsingleton-purpose key、Transactionは`service.control-plane-rebaseline-publisher`／`role.control-plane-rebaseline-publisher`のsingleton-purpose keyで署名する。subject hash、Signer／Role、型別時刻、current assignment／Key／Role／revocationを共通署名規則で再検証する。`CurrentControlPlaneBaselineBindingV1`はProduct operational snapshot内のinline immutable valueであり、genesisはbootstrap branch、両publication kindはrebaseline branchを生成し、通常state／lifecycle transitionはbyte-exactに保持する。branchのApproval／Envelope型、active definition hash、Transaction `state=publication_ready`を再検証し、2回目以降も直前current bindingをsource coreへそのまま束縛する。
...
| MCD logical contract | `McdContractRefV1.version` positive `uint32`＋exact `contract_set_hash` | normative意味変更ごとのexact Schema／record解決 |
| Contract set | SHA-256 | 一つのBuildが使用するexact closure |
| Native ABI profile | major＋exact profile hash | Native binary load可否 |
| Artifact identity | `ArtifactRefV1.schema_version` `uint32`＋content SHA-256 | exact immutable bytes。format major／minorはkind固有Manifestで解決 |
| Capability activation | closed state | 利用可能性。Versionではない |

この節はRef wire shapeを再定義しない。current唯一の正本は[Executable Contracts](../architecture/02-foundation/executable-contracts.md#5-mcd共通envelope)の`McdContractRefV1 {id, version, contract_set_hash}`、`ArtifactRefV1 {artifact_kind, schema_version, sha256}`、`Sha256DigestV1`である。`version`は同じlogical MCD IDで1から単調増加し、normative Field、受理集合、意味、Owner、Authorityの変更ごとにexact `N+1`とする。同じID／versionの別member bytes、version再利用、rootなしschema hashだけの参照を拒否する。Contract Set member hashがcanonical MCD Schema／record bytesを、set rootがそのexact closureを既に束縛するため、`type_major`、`revision`、`schema_sha256`、`contract_set_sha256`という別名Fieldを`McdContractRefV1`へ重複追加しない。

`ArtifactRefV1.sha256`は完成immutable bytesだけを指す。format major／minor、format schema hash、content size、entry countはArtifact kind固有のcanonical Manifest／Envelopeで束縛し、Refへ別名複写しない。将来Ref自身のwire shape変更が不可避なら、Executable Contracts Ownerが`McdContractRefV2`または`ArtifactRefV2`を新majorとして定義し、全consumer、offline migration、retained V1 reader、positive／negative fixtureを同じFoundation definition migrationで導入する。同名V1の「clean置換」またはDomain文書での先行定義を禁止する。
...
Unknown Field判定は、Envelopeに記録されたexact MCD refまたはArtifact refからContract Set member／kind固有format schemaを解決した後に行う。そのSchemaにないFieldは拒否する。新readerはsupport window内の旧exact MCD versionまたはformat revision decoderとfixtureを保持する。Saveは同じepoch内の全Shipping revisionを保持する。旧readerへ未来versionを読ませない。

Live IPCでversion negotiationが必要な場合、双方が列挙したexact `McdContractRefV1`またはkind固有`{format_version, format_schema_hash}`集合の最大共通要素を、Compatibility Registryの明示orderで選ぶ。Range、`latest`、Contract set rootなしMCD versionは使わない。永続artifactはOwnerのcurrent exact refを明記して出力し、暗黙downgradeしない。旧version writerが必要なら専用encoder、fixture、support期限をCompatibility Registryへ登録する。
...
- Manifestは自身のdigestまたは後段artifactのdigestをFieldに持たない。Executable Contracts所有の`ManifestArtifactEnvelopeV1`がexact `manifest_ref: ArtifactRefV1`、`payload_root_sha256`、`entry_count`、`trust_profile_ref`を外側で束ねる。Manifest digestは`manifest_ref.sha256`だけに置き、別Fieldへ複写しない。Signature refはEnvelopeへ入れず、既存`MirakanSignedRecordV1`がEnvelopeの`ArtifactRefV1`を外側からbindする。
...
共通`UnsignedApplicationPackageV1` envelopeは`assembly_manifest_ref: ArtifactRefV1`、`payload_root_sha256`、entry count、Target固有envelope refを持つ。Manifest digestは`assembly_manifest_ref.sha256`だけに置く。Manifest、共通envelope、Target固有envelopeはpayload entryへ含めず、8.5の外側hash規則で束ねる。
...
| `McdContractRefV1`、`ArtifactRefV1`、`Sha256DigestV1`、`ManifestArtifactEnvelopeV1` | Executable Contractsだけが構造を定義する。Compatibility／Evolutionはexact importし、同名V1を再定義またはin-place置換しない |
| `FoundationDefinitionClosureV1` | Executable Contracts。Contract Set、Owner Identity Registry、Named Algorithm Registryの三rootだけを閉じる。Architecture GovernanceはLocal Schema Catalogへexact schemaを登録してBaseline／Rebaseline Coreで消費するが、closure構造またはhash domainを再定義しない |
...
Foundation correction後の追加Control Plane関連contract型は29型からexact 30型へ増える。増分は`FoundationDefinitionClosureV1`一型だけであり、同型はlogical IDを持たず完成content ref／hashで同定するためContent ID Registryのexact 32件は変更しない。Owner Identity Registry、Named Algorithm Registry、六Trust Registryをこの件数へ重複加算しない。
