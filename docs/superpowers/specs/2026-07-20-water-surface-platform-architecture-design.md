...
| C2 | `lake`／`spline_river`／`ocean`、解析的波、current map、水深、foam mask、probe＋SSR、underwater、CPU Surface Query、buoyancy連携、quality variant |
...
| Target | 責務 |
|---|---|
| `mira_water_contracts` | value type、enum、Snapshot、Query、Diagnostic |
| `mira_water_core` | body意味、wave／flow評価、priority、budget |
| `mira_water_compiler` | Source validation、mesh／depth／query Artifact生成 |
| `mira_water_runtime` | instance、Snapshot、quality、lifecycle |
| `mira_water_query` | CPU surface／depth／flow query。Physics Backend非依存 |
| `mira_water_render` | Water packet、Material binding、Render Graph template |
| `mira_water_authoring` | ChangeSet projection、preview、cost、cook request |
| `mira_water_qualification` | golden、CPU／GPU一致、performance、fault fixture |

`mira_water_query`はRenderer、D3D12、Vulkan、Metalへ依存しない。`mira_water_render`はPhysics Worldへ依存しない。
...
Hard上限超過Sourceはcookを拒否する。Runtime visibility超過は`priority desc, screen_influence desc, body_id asc`の末尾を描画から除外してDiagnosticを発行するが、Water VolumeとGameplay Queryは維持する。

Quality fallbackは波成分数、normal octave、SSR resolution、foam resolution、underwater sample数の順で明示variantを選ぶ。Body boundary、Gameplay water level、flow direction、Visual Styleを変更しない。
