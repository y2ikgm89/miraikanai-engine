...
[Product Plan §5.3](../00-product/product-plan.md#53-c2-production-exact-product-boundary)はC2のexact Android runtime Targetを`target.android.mobile@1`、public publication baselineを`package-profile.android.play`のAAB＋Google Playとする。本書はそのProduct selectionを変更せず、AAB、Signing、Play upload／review／public read-back、install-time content、Project C++／Project Shader artifact、device qualificationを所有する。internal／closed test track、uploadまたはreview approvalはqualification inputであり、public publicationの代用ではない。offline completionとrequired 2D／3D Reference pathに必要なcontentはbase moduleまたはinstall-time Asset packへ含め、fast-follow／on-demandだけを必須pathにしない。

根拠: official-spec／project-decision — [Google Play target API要件](https://developer.android.com/google/play/requirements/target-sdk)は2026-08-31から新規app／updateへAndroid 16／API 36以上を要求する。[16 KiB page support](https://developer.android.com/guide/practices/page-sizes)はNDK r28以上のdefault alignmentとprebuiltを含むELF／package検査、[Android Vulkan Profiles](https://developer.android.com/ndk/guides/graphics/android-vulkan-profile)は複数Profileと観測coverage、[48 dp touch target guidance](https://developer.android.com/design/ui/mobile/guides/foundations/accessibility)はaccessibility guidanceを示す。minimum API 29、NDK r29、required baseline `VP_ANDROID_vulkan_profile_2022`（AVP API version 1.1.106）、optional high `VP_ANDROID_vulkan_profile_2025`（AVP API version 1.1.128）、対応Device範囲はMiraikanaiの候補判断であり、Googleの既定値や市場coverage保証ではない。
...
閾値未承認、AAB未upload、export欠落、filter不一致、snapshot失効、coverage未達のいずれかではAPI 29を市場coverage達成として扱わず、Product C2のAndroid Target qualificationをfail closedにする。公開Web上の一般的なOS分布、Device Catalogのmodel数、target API policy、別appの値を代用しない。minimum SDKを変更する場合はToolchain baseline、manifest、device matrix、Product Definition、Package requirementを同じArchitecture revisionで更新する。
