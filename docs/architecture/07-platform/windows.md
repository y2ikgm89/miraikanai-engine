...
| AI Production Tooling Host process group（exact executable set未決定） | Orchestrator／Context／Route／Workflow／Agent logical roleの隔離composition。Provider Credential、Project write、Signer authorityをunionしない |
...
Editor起動child tree全体は`EditorSessionJob`でkill-on-job-closeとchild process policyを適用するが、AI／Compiler／WorkerまでEditor memoryへ誤計上しないため、このroot Jobへaggregate memory limitを設定しない。EditorHost、GameHost、Source／Asset／Shader Worker、AI Production Tooling Host process group、Package Service、Crash Collectorは[Runtime performance／capacity](../04-runtime/performance-capacity.md)のprocess group、allocator tag、working-set telemetry、sandbox capacityへ個別にchargeする。AI logical roleをco-locateする場合もProvider Adapter／local inference／external BrokerはAI Security Profileのseparate sandbox boundaryを保ち、Credential、filesystem、network、Signer、Project writeのauthorityをunionしない。Workerはsuspended起動後にtask別nested Jobへ割り当ててからresumeし、GameHost終了時にProject C++ job、GPU、Audioをjoinし、timeout時はProcess単位で終了する。
...
| Project生成物 | [Naming／Project Layout](../02-foundation/naming-project-layout.md)の`derived/`、`intermediate/`、`packages/`。legacy `build/` rootを作らない |
| User cache | `FOLDERID_LocalAppData`下のEngine／user cache。Project SourceまたはProject生成物のauthorityにしない |
