...
[Product Plan §5.3](../00-product/product-plan.md#53-c2-production-exact-product-boundary)はC2のexact Apple runtime Targetを`target.apple.mobile@1`、public publication baselineを`package-profile.apple.bundle`＋App Storeとする。本書はそのProduct selectionを変更せず、App bundle、Signing、TestFlight qualification、App Store upload／review／public read-back、install-time content、Project C++／Project Shader artifact、iPhone／iPad device qualificationを所有する。TestFlight、uploadまたはreview approvalはqualification inputであり、public publicationの代用ではない。`package-profile.apple.managed-assets`はiOS／iPadOS 26以上を要求する別variantで、deployment target 17.0のC2 baselineへ混在させない。

根拠: official-spec／project-decision — [Xcode SDK／system requirements](https://developer.apple.com/xcode/system-requirements)はXcode 26.6のiOS／iPadOS 26.5 SDKとdeployment support範囲、[App Store submission requirements](https://developer.apple.com/app-store/submitting/)は2026-04-28以降のiOS／iPadOS 26 SDK以上、[Apple-hosted asset packs](https://developer.apple.com/help/app-store-connect/manage-asset-packs/overview-of-apple-hosted-asset-packs/)はiOS／iPadOS 26以上のManaged Background AssetsとTestFlight／App Store routeを示す。[Accessibility HIG](https://developer.apple.com/design/human-interface-guidelines/accessibility)と[Typography HIG](https://developer.apple.com/design/human-interface-guidelines/typography)はUI guidanceを示す。deployment target 17.0、対応Device範囲、C2 bundle baseline、managed-assetsの別variant化はMiraikanaiの候補判断であり、Appleの一般推奨値または市場coverage保証ではない。
...
offline shader pathは[Project Shader](../06-rendering/project-shader.md)とToolchain lockのProfileに従い、portable HLSLからintermediate、MSL、Metal libraryへ変換し、Apple専用`ProjectShaderArtifactSetV1`としてApplication bundleへ格納する。Package validatorはApple Target Profile、Engine baseline、`ProjectShaderQualificationReceiptV1`、artifact／interface hashを照合する。Shippingへcompiler、shader source、authoring reflectionを含めない。unqualified Target-limited Module／Technique／Materialはexplicit fallbackとvisual diffなしにCookしない。C2 Product DefinitionはAppleのbounded Project C++とProject Shaderをrequired Target bindingとして選ぶため、Engine-owned／事前Qualification済みartifactだけをProject Source supportへ数えない。Apple Project Sourceのauthoring、Qualification対象化、Runtime選択、static link／Metal artifact packageは同じCandidateのfresh Apple Target Qualificationがmaterializeするまで`not_activated`であり、Architecture上のC2 requirementを利用可能性またはpackage成功として表示しない。
...
`package-profile.apple.bundle`は`install`をapp bundle、`essential | prefetch | on_demand`を同名policyのself-hosted unmanaged deliveryへ写像する。C2のoffline completionとrequired 2D／3D Reference pathに必要な全Assetは`install`へ含め、`essential | prefetch | on_demand`だけを必須pathにしない。deprecated delivery mechanismを新規採用しない。
...
現行Product C2 Apple Shipping RuntimeはAI structured data生成／mutation、provider network call、generated contentのdownload／loadを許可せず、[Mobile Common](mobile-common.md)の`ProjectMobileSpecV1.runtime_generation_policy=deny_all`でProject／Save／authoritative Worldを不変に保つ。positive Runtime generation、content safety、生成content rollbackは`future.capability.runtime-structured-data-generation`を採択する新しいActive Product Definition／Mobile spec revision、Apple Target binding、専用Authority／Threat Model、fresh Target Qualificationでだけ追加する。post-review executable code／shader pipelineのdownload／load禁止はその後も維持する。
