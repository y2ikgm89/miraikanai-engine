...
- Material PanelのGraph modeは[Materials](../06-rendering/materials.md#21-material-functionsubgraph)の`MaterialDefinitionV1` typed Graphと`MaterialFunctionDefinitionV1`だけを表示する。Function callはexact Function revision／Port Stable IDを示し、inline展開表示、breadcrumb、comment、group、reroute、layoutをMaterial意味またはAI targetにしない。Material Function authoringが`not_activated`の間はinspect、dependency、stale／disabled reasonだけを表示し、既存`operation.material.edit_graph`からFunction作成または参照元一括更新を生成しない。
- VFX Graphは[VFX Authoring](../06-rendering/vfx-authoring.md)の同一`VfxSystemDocumentV1` typed DAGを表示する。Beginner Module StackとGraphは同じSourceのprojectionであり、相互変換で別Asset、別Node ID、別revisionを作らない。qualified `VfxExtensionOperatorV1`も同じGraph Patternを使うが、extension QualificationまたはOperation activationをEditor表示から発生させない。
- Camera Rig Graphは[Camera](../06-rendering/camera.md)のtyped Rig DAGを表示し、`ProjectCameraRigNode`をoffline-linked Native Game Module refとして投影する。JIT、dynamic load、script call、native object、Function pointerをGraph nodeまたはDetailsへ公開しない。
- AudioのGraph表示は[Audio](../07-platform/audio.md)のMixer Bus Graphだけを対象とし、Sound Cueのbounded treeを汎用Graphへ変換しない。Engine Catalog外DSP、Project C++／HLSL／plugin DLL Nodeをempty placeholderまたはgeneric custom nodeとして先行表示しない。
...
- 初期`fixture.graph-timeline.reference@1`はGameplay、Animation、Topology、Render、Causality GraphとAnimation／Debug Timelineだけを一つのvisual／semantic基準にする。50,000 node、100,000 time itemでもviewport＋bounded queryだけをmaterializeし、total／omitted／continuationをHuman、UIA、AIで一致させる。Material／VFX／Camera／Audioは上記の共通Graph PatternへのOwner bindingを設計固定するが、初期五Graph、166 coverage entry、九Panel、既存Manifestへ追加しない。各OwnerのSource schema、read／write authority、Operation activation、Domain fixtureが完成した後の別Reference Definitionだけが、同じPatternの追加coverageを提案できる。共通Patternへの記載をPanel、Capability、Operation、Fixtureのcurrent activationへ読み替えない。
...
| `fixture.graph-timeline.reference@1` | 初期五Graph、Animation／Debug Timeline、50,000 node、100,000 time item、Source／Runtime／recorded／gap。Material／VFX／Camera／Audioの将来Owner coverageを含まない |
