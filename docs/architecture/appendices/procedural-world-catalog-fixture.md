...
このinternal Operationを次の一件の完全recordとしてcurrent Contract setへ登録する。これは[Executable Contracts](../02-foundation/executable-contracts.md)が所有するcanonical `McdOperationContractV1` schemaのcomplete instanceであり、schemaの再定義ではない。
