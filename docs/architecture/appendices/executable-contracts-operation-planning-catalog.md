...
`type.operation.mutation_authorization_binding@2`のpayload schemaは[AI Security／Approvalのcanonical `MutationAuthorizationBindingV2`](../01-governance/ai-security-approval.md#mutation-authorization-binding-v2)だけを正本とする。本書へSchema blockをmirrorせず、Contract projectionは正本Schemaのcontent referenceを入力に生成する。生成器とSchema Artifactが存在しない現在は、このTypeをcurrent materialized MCDと表現しない。
...
`MIRAKAN_OPERATION_REQUEST_V2`と`MIRAKAN_OPERATION_INTENT_V2`は2026-07-27時点でEngine、Project、Tool catalog、Receipt storeへ一度もActivation／materializationされていない設計契約であるため、version名を維持してActivation前に修正できる。レビュー対象だった循環形のV2 bytesは永続artifactではなく、migration sourceとして受理しない。既存V1 retained artifactだけはoffline-only migration recordで扱う。この二段階DAGとnamed bindingは設計制約であり、本書は実装Taskまたは作業順序を定義しない。旧循環shapeの互換readerは作らない。

旧domain `MIRAKAN_OPERATION_REQUEST_V1`はretained Receiptの検証とoffline migration inputだけに許可する。候補migration definitionでは状態変更Operationを半登録せず、次の非current migration algorithm recordだけを定義する。
