...
Foundation MCDとのtarget bridgeを次に固定する。`capability.runtime.scheduling@1`のMCD candidateが将来同じContract setへmaterializeされる場合、その`supported_targets`は`{target.android.mobile, target.apple.mobile, target.headless.host, target.windows.desktop, target.windows.editor}`である。本節の同ID候補行はこの五Targetすべてを`required`で持ち、set equalityを必須にする。MCD `status=active`は将来契約がContract setで参照可能になった状態だけを意味し、`ProductOperationalStateSnapshotV1`のActivation rowを変更しない。現RepositoryのMCD／Product Registry instanceは存在せず、Candidate／Qualification／Production Receiptがないrowは`not_activated`であり、MCD candidateをReceiptとして扱わない。

`capability.authoring.command_gateway@1`はTrusted Serviceのauthority境界を記述するcurrent C0 Capabilityで、Product選択、Phase label、Target shippingを表すCapabilityではない。このためProduct `CapabilityRegistryV1`に行を持たないことが正規であり、欠落補完、暗黙Product Activation、利用可能表示を行わない。Serviceの起動可否は[AI Security／Approval §7.3](../01-governance/ai-security-approval.md#73-trusted-service-isolation)のservice-authority branch、すなわちcurrent Signer Policy operation集合、Service allowlist、Trust Head／closure、executable／Profile／Contract set／Targetのfresh Technical Qualificationをset equalityにして判定し、Product row不存在を起動不能またはProduct support claimへ読み替えない。initial designにはoffline migration用Capability、Service、ProfileまたはProduct rowを登録しない。最初の公開後にCompatibility Ownerのconsumer inventoryがversioned migrationを要求する場合だけ、新しいatomic ChangeSetでProduct-facing capabilityとTarget Gateの要否を判断する。
...
ProductExecutionTargetQualificationProjectionV1
  entries[]:
...
FutureTargetClosureRegistryV1
  entries[]: future_capability_ref,
             closure = {kind: single_target}
                     | {kind: target_role_bundle,
                        bundle_profiles[]: FutureTargetRoleBundleProfileV1}
...
FuturePortfolioDefinitionBundleV1 // independently approved planning-only hash domain
  ...
  future_target_closure_registry_ref
...
FutureToActivePromotionManifestPayloadV1
  ...
  future_capability_id
  destination_active_product_definition_sha256
  target_closure_binding:
    {kind: single_target,
     target_profile_ref, target_profile_sha256}
    | {kind: target_role_bundle,
       bundle_profile_id, bundle_profile_content_hash,
       role_target_bindings[1..8]:
         {role_id,
          target_profiles[0..16]:
            {target_profile_ref, target_profile_sha256}}}
  promoted_active_ids[]: {registry_id, row_kind, logical_id, migration_kind}
  role_activation_keys[]: {role_id, capability_id, target_id}
...
planning-only entryの追加、説明修正、claim vocabulary更新、Future dependencyまたはTarget closure変更はFuture membership revisionだけを更新し、current active Product Definition、WP lifecycle、Activation、Phase Evidenceを失効させない。Future entryをactiveへ移すChangeSetだけがdestination Active Product Definitionを変更し、destination `ControlPlaneRebaselineApprovalV1`／Envelope／Transaction、Product migration Decision、full-reset state migrationを必要とする。初回Bootstrap Approvalを再発行または後続Definitionへ流用しない。各bundle profileの`active_prerequisite_bindings[]`は親Future行の`prerequisite_capability_refs[]`、`common_future_prerequisite_bindings[]`は`prerequisite_future_capability_refs[]`、`claim_role_requirements[]`は`excluded_current_product_claims[]`とそれぞれset equalityにする。`additional_future_prerequisite_bindings[]`はcommon集合とdisjointで、同じapproved Future closure内のexact IDだけを参照する。全profileのcommon／additional Future edge unionで自己参照、missing、cycleを拒否する。claim requirementのrole IDは同じprofile内のroleへ解決し、各required roleを重複なしnon-empty集合とする。`single_target`はactive／Future前提を同一exact Target、`target_role_bundle`は選択profileのactive binding、Future role binding、bundle前提role mappingで検証し、必要な全Targetでproductionになるまでconsumer entryをactive migration候補にしない。

Future起源のactive migrationでは`future_promotion_inputs[]`にsource Future closure hash、その時点のcurrent完成`FuturePortfolioApprovalV1` ref／hash、Future ID、追加／保持するactive Registry ID、`FutureTargetClosureRegistryV1`から解決したtarget closure bindingを列挙し、migration authorization Decision、row migration manifestと一緒に承認する。Approvalのpayloadが同じsource Future closure hashを承認し、current approval headで有効、非失効でなければならない。同一definition hashのrenewalでもapproval ref／hash／sequenceを省略しない。`single_target` bindingはexact Target Profile一件、`target_role_bundle` bindingはprofile ID／content hash、profileの全roleとset equalityな`role_target_bindings[]`、roleごとのexact Target Profile ref／hash集合を持つ。role bindingはrole ID、各`target_profiles[]`はTarget Profile refのunsigned UTF-8 byte順にし、optional 0件をrole省略で表現しない。role cardinality、全pair relation、active／common Future／additional Future prerequisite binding、claim role requirementのclaim／role集合を再検証する。

`single_target`の`role_activation_keys[]`はexact `[]`とする。`target_role_bundle`のActivation closureは、source Futureから昇格するdestination Capability、`active_prerequisite_bindings[]`のactive Capability、common／additional Future前提をDAG末端まで再帰展開した全destination／active prerequisite Capabilityを各binding先roleのexact Targetへ投影する。single-target前提はbinding先各Targetで同じ展開を続け、bundle前提は有効なPromotion Manifestの再帰済みrole keyをrole mapping後のconsumer roleへ写す。重複除去した全`{role_id,capability_id,target_id}`と`role_activation_keys[]`をset equalityにし、直接前提のdestinationだけで打ち切らない。bundle前提はrole mapping後の全required roleを含める。各keyはrow migration manifestの追加／保持IDとdestination `CapabilityRegistryV1.target_bindings[]`へ解決しなければならない。profileの一部、別profile roleの混成、unbound promoted ID／Target、同roleのmissing／extra key、optional 0-Target roleへの架空keyを拒否する。配列はFuture ID、内側IDは`{registry_id,row_kind,logical_id}`、role keyは`{role_id,capability_id,target_id}`のunsigned byte tuple順で、duplicate、source entry不存在、row manifestにないID、`removed` IDを拒否する。migration完成後、専用`service.future-to-active-promotion-manifest-publisher`／`role.future-to-active-promotion-manifest-publisher`／singleton purpose `future_to_active_promotion_manifest`が、migration inputのsource Approval ref／hashとtarget closure bindingをbyte-exactに複写した同じmappingと完成migration wrapper ref／hashを`FutureToActivePromotionManifestV1`として署名する。`promotion_manifest_id`は同Fieldを除くpayload JCS hashから`urn:mirakan:future-promotion:sha256:<lowercase-hex>`として導出し、`signed_record.issued_at=payload.generated_at`、revocation snapshotをexact一致させる。これをFuture ID、Target-role closure、active IDsの唯一のorigin mappingとし、名前一致や説明文から由来を推測しない。

`excluded_current_product_claims[]`はplanning中の禁止claimであり、active migrationだけで自動解除しない。解除は、valid `FutureToActivePromotionManifestV1`が列挙するactive Capability、選択したsingle TargetまたはTarget-role bundleの必要全Activation `production`、fresh Release Receipt、effective open Critical Risk 0、kind `future_claim_release_product`／subject kind `future_product_claim_release`のfresh R4 `ProductOperationalDecisionV1`を閉じた`FutureProductClaimReleaseV1`を必要とする。Claim payloadのoperational snapshot ref／hashは発行時監査とcommit条件であり、validatorはそのinline rowを`activation_keys[]`でlookupしてproduction、same Candidate、freshness、promotion manifest内Capability／Target-role closureを再計算する。`target_role_bundle`ではPromotion Manifestの`role_activation_keys[]`を`{capability_id,target_id}`へ投影して重複除去した集合と`activation_keys[]`をset equalityにし、元のrole付き集合でも全role coverageを再検証する。さらに`released_claims[]`各claimを選択profileの`claim_role_requirements[]`へexact解決し、列挙された全roleの`role_target_bindings[].target_profiles[]`がnon-emptyであることを要求する。profile ID／hash、role Target set、active／common Future／additional Future prerequisite、claim required roleのいずれかが欠ければ該当claimをunreleasedにする。一roleだけの成功や別profileのTargetを合成せず、optional roleが空のままそのroleを必要とするclaimを解除しない。独立しない`activation_row_refs[]`を発明しない。`claim_release_id`は同Fieldを除くpayload JCS hashから`urn:mirakan:future-claim-release:sha256:<lowercase-hex>`として導出し、`role.future-claim-release.r4`とsingleton purpose keyで署名する。`signed_record.issued_at=payload.issued_at`、revocation snapshot、purpose、Roleをexact一致させる。

Claim release chainは`future_capability_id`ごとに一つで、初回だけprevious両Field=nullかつ`claim_release_sequence=1`、後続はcurrent完成Release wrapper ref／hashとexact `N+1`を持ち、per-Future current-head CASで一件だけ進める。branch、sequence gap、別Futureのprevious、filesystemのlatest探索を拒否する。read-time `effective_release`は発行時snapshotをcurrentと見なさず、Product operational current headから同じ`activation_keys[]`を毎回key lookupし、current headのactive definition hashがpayloadのdestination hashと一致し、全行がproductionかつeffective availability、same Candidate、fresh non-revoked Release Evidence、Risk 0、current Decision／Signer validityを満たす場合だけ列挙claimを解除する。bundle claimではcurrent Promotion Manifestのprofile ID／hash、role Target集合、claim required non-empty roleも毎回再検証する。無関係transitionで条件が不変ならReleaseは有効を維持するが、Definition migration、Target降格、Candidate変更、required roleの空集合化、Evidence expiry／revocation、Decision／Release期限切れで即時effective unreleasedへ戻す。Marketing文言やCapability存在だけをrelease Receiptにしない。

`released_claims[]`はsource Future rowの`excluded_current_product_claims[]`に含まれ、同じsource Future closureの`ProductClaimDefinitionRegistryV1`へ解決するnon-empty `claim_id` subsetだけを許し、全解除を主張する場合はset equalityを必須とする。bundleでは各claimが選択profileの`claim_role_requirements[]`にexact一件存在し、そのrequired role集合を満たすことも必須とする。unknown／duplicate／別Future claim、canonical label、Marketing自由文字列、claim requirement不在、optional role空集合の過大主張を拒否する。`single_target`の`activation_keys[]`は、source Futureのdestination、active prerequisite、全common Future prerequisiteをDAG末端まで同じ選択Targetへ再帰展開したCapabilityについて、destination `CapabilityRegistryV1.target_bindings[]`の`required | optional` keyを重複除去したexact集合とする。`target_role_bundle`では前段の再帰済みrole付き集合の投影を使う。呼出元が直接またはtransitive prerequisite、Target、roleを省略できない。`production_receipt_refs[]`はそのcurrent Activation rows、`CapabilityTargetActivationBindingRegistryV1`、該当`ProductReleaseGateRegistryV1`をproductionで支えるfresh Receiptのexact closureで、missing／extra／別Candidate／別Target／別roleを拒否する。部分claimでもActivation／Production Evidence closureを縮小しない。
...
|---|---|---|---|---|---|---|---|---|---|
| `capability.authoring.ai-core` | `wp.authoring.ai-core` | `requirement.product.ai-authoring-mvp-a; requirement.product.ai-genre-neutral-authoring` | `phase.ai-authoring-mvp-a` | `fixture.product.genreless-ai-project; fixture.product.shooter-2d` | `planning.operation_family.authoring_discovery; planning.operation_family.game_system_discovery; planning.operation_family.world_discovery; planning.operation_family.gameplay_definition_authoring; planning.operation_family.asset_authoring; planning.operation_family.authoring_changeset_execution; planning.operation_family.game_intent_understanding; planning.operation_family.game_experience_iteration; planning.operation_family.game_production_read` | `windows_editor_for_artifact_target` | `all_activation_declared_external_safe` | `risk.product.ai-tool-safety-code-owner` | `policy.product.activation.promotion.v1; policy.product.same-candidate.v1` |
...
| `capability.product.ai-authoring-mvp-a` | `wp.product.ai-authoring-mvp-a` | `requirement.product.ai-authoring-mvp-a; requirement.product.ai-genre-neutral-authoring; requirement.product.authoring-roundtrip; requirement.product.mvp-completion` | `phase.ai-authoring-mvp-a` | `fixture.product.genreless-ai-project; fixture.product.shooter-2d` | `planning.operation_family.authoring_discovery; planning.operation_family.game_system_discovery; planning.operation_family.world_discovery; planning.operation_family.gameplay_definition_authoring; planning.operation_family.asset_authoring; planning.operation_family.authoring_changeset_execution; planning.operation_family.game_intent_understanding; planning.operation_family.game_experience_iteration; planning.operation_family.game_production_read; planning.operation_family.build_candidate_test; planning.operation_family.build_device_play_debug_task` | `windows_editor_for_artifact_target` | `all_activation_declared_external_safe` | `risk.product.ai-tool-safety-code-owner` | `policy.product.activation.promotion.v1; policy.product.same-candidate.v1` |
| `capability.product.external-agent` | `wp.product.external-agent` | `requirement.product.external-agent-boundary` | `phase.external-agent` | `fixture.product.external-agent-proposal` | `planning.operation_family.authoring_discovery; planning.operation_family.game_system_discovery; planning.operation_family.world_discovery; planning.operation_family.gameplay_definition_authoring; planning.operation_family.asset_authoring; planning.operation_family.authoring_changeset_execution; planning.operation_family.game_intent_understanding; planning.operation_family.game_experience_iteration; planning.operation_family.game_production_read; planning.operation_family.build_candidate_test; planning.operation_family.build_device_play_debug_task` | `headless_proposal_only` | `proposal_subset_excluding_trusted_internal` | `risk.product.ai-tool-safety-code-owner` | `policy.product.activation.promotion.v1; policy.product.same-candidate.v1` |
...
27 planning familyはpositive十行のunionへexact全件接続する。`game_intent_understanding`、`game_experience_iteration`、`game_production_read`のexact三件はAI Core、AI Authoring MVP-A、External Agent行へ接続し、Proposal／read subsetだけを外部投影可能にする。Human Gameplay Approval、Project Commit、Source Promotion、ActivationまたはReleaseを三familyへ追加しない。`rendering_aa_discovery`、`lighting_discovery`、`post_process_discovery`、`math_semantic_authoring`、`lod_authoring`のexact五件はGeneral Production 3D行だけが追加要求し、同rowがdeferredの間はProduct bindingが存在してもActivation／Provider公開／Capability昇格を許可しない。これらをMVP-AまたはGeneral 2Dへ暗黙追加せず、逆にC2 3D移行時に説明名だけで省略しない。matrix compilerは27 family集合と全positive rowのfamily unionをset equalityにし、unboundまたはunknown familyをActive Definition compile errorにする。

`deny_only_capability_refs[]`のtarget exact一件候補は`capability.product.runtime-generation-boundary`である。同Capabilityと`wp.product.runtime-generation`のpositive `execution_surface_bindings[]`はexact `[]`とし、将来materializeする`fixture.product.runtime-generation-denial`でRuntime packageにOrchestrator、Provider credential、MCP server、Source Worker、Compiler、Signer、write-capable Gatewayが0件であることだけをQualificationする。将来Runtime generationを追加する場合は本行をsilent変更せず、Future Portfolio→Active Definition migration、Threat Model、Target別Operation family、Signer／Service／Receipt closure、positive／negative fixtureを一つの変更として承認する。
...
このRegistryによりTier、WP existence、provided fixtureだけからActivationを推測しない。optional rowも同じEvidence／execution mappingを持つがProduct aggregateには含めない。current 27 planning family／207候補は全て`not_activated`であり、上表のpositive十CapabilityはWP状態にかかわらず現時点で`qualified`になれない。
...
このindependenceはMiraikanai Engine自身のProduct operational state、WP acceptance、definition migration、baseline rebinding、risk／claim releaseを自己認証させないためのEngine-development governanceである。[Product Plan](../00-product/product-plan.md)の第三者Developer journeyまたは[AI Security／Approval](../01-governance/ai-security-approval.md)のR2／R3 Project authoring quorumを一律にR4へ昇格しない。単独主体は候補、Diff、Build／Test Evidenceを作成できるが、同じ主体がrequester、実装worker、Evidence issuerまたは複数Role／Keyを兼ねてもこのDecisionのindependent signerになれない。必要な独立主体が存在しなければ該当Product transitionは未承認のままであり、架空Role、AI reviewer、publisher signatureまたはlocal成功を代用しない。
...
`gate.product.phase-3-manual-2d`は互換維持用のopaque IDであり、本RevisionではShooter 2D technical fixtureのTitle→Result、Save／Load／Replayだけを評価する。[Product Plan §5.1](../00-product/product-plan.md#51-first-playable-outcome)と[RPG Genre Pack](../08-packs/rpg.md)が要求するcompact 2D command RPG ReferenceのEvidenceではなく、`requirement.product.manual-first-playable-2d`、Product-facing First Playable acceptanceまたはRPG Capabilityを満たさない。RPG Fixture、RPG Owner Work Package、Capability、Phase Gate、Target bindingを同じ承認済みDefinition変更で登録し、両Ownerのnon-substitution closureをvalidationするまで、Phase 3のProduct-facing outcomeは評価不能とし、Shooter／Genre-neutral Receiptから補完しない。
...
| `requirement.product.cpp23-public-headers-cross-target` | `mirakan.arch.cpp23-modules` | `cross_target_public_header_surface` | `diagnostic.product.cpp23-public-headers-cross-target-incomplete` |
| `requirement.product.cpp23-shipping-cross-target` | `mirakan.arch.cpp23-modules` | `cross_target_shipping_language_mode` | `diagnostic.product.cpp23-shipping-cross-target-incomplete` |
...
| `fixture.product.shooter-2d` | `mirakan.arch.pack-shooter` | `requirement.target.mobile-runtime-2d; requirement.product.ai-authoring-mvp-a; requirement.product.authoring-roundtrip; requirement.product.title-to-result; requirement.product.save-load-replay; requirement.product.mvp-completion; requirement.product.project-source-activation; requirement.product.c2-2d-coverage` | `target.windows.editor; target.windows.desktop; target.android.mobile; target.apple.mobile` | 300 |
...
| `fixture.product.cpp23-public-headers-cross-target` | `mirakan.arch.cpp23-modules` | `requirement.product.cpp23-public-headers-cross-target` | `target.headless.host; target.windows.editor; target.windows.desktop; target.android.mobile; target.apple.mobile` | 1800 |
| `fixture.product.cpp23-shipping-cross-target` | `mirakan.arch.cpp23-modules` | `requirement.product.cpp23-shipping-cross-target` | `target.headless.host; target.windows.editor; target.windows.desktop; target.android.mobile; target.apple.mobile` | 3600 |
...
| 3 | `phase.manual-2d` | `requirement.product.core-pack-independence` | `wp.gameplay.core-c1; wp.runtime.timer; wp.rendering.world-2d; wp.rendering.camera-2d; wp.simulation.collision-2d; wp.simulation.physics-2d; wp.simulation.animation-2d; wp.navigation.core; wp.runtime.ecs-e5-2d-integration; wp.gameplay.reusable-features-c1; wp.domain.shooter-2d` | `gate.product.phase-3-manual-2d; gate.product.phase-3-genreless-core` |
...
Phase 3のopaque legacy `gate.product.phase-3-manual-2d`は`fixture.product.shooter-2d`のTitle→Result／Save内部候補だけを評価し、compact 2D RPG、`requirement.product.manual-first-playable-2d`、C1 First PlayableまたはProduct-facing Phase exitを評価しない。`phase.manual-2d`表示名、order 3またはShooter successからRPG bindingを補完しない。Phase 5とPhase 9はこのRegistry行を唯一のscheduling identityとし、本文上の見出しや番号だけで存在を表現しない。Phase 5の`gate.product.phase-5-external-agent`はProposal-only authorizationだけを評価し、Native／Shader Sourceを作成またはactivateしない。`gate.product.phase-5-project-source-activation`はCode owner付き新規Source laneだけを評価し、external Client接続の成功を代用しない。同GateのProject Shader部分は`bounded_hlsl`と`typed_ir`の二sub-closureを必須とし、現行Registry revisionのまま片方だけへ縮退しない。Phase 8の三Genre gateは`capability.product.general_production_2d`とbundled Reference Game coverageだけを評価する独立trackで、Generic Engine Release、C++23 Shipping qualification、production-release bindingをblockしない。Phase 9はdeny-only C0 boundaryであり、Phase 8 coverageの完了またはC2 Product Capabilityへ依存せず、許可されたpositive Runtime structured-data generationは`future.capability.runtime-structured-data-generation`の`planning_only`境界に留める。
...
| `gate.product.phase-3-manual-2d` | `phase.manual-2d` | `fixture.product.shooter-2d` | `requirement.product.title-to-result; requirement.product.save-load-replay` | `target.windows.desktop` | `policy.product.same-candidate.v1` | `policy.evidence.target-device.v1` |
...
| `gate.product.phase-8-cpp23-public-headers-cross-target` | `phase.production-capability` | `fixture.product.cpp23-public-headers-cross-target` | `requirement.product.cpp23-public-headers-cross-target` | `target.headless.host; target.windows.editor; target.windows.desktop; target.android.mobile; target.apple.mobile` | `policy.product.same-candidate.v1` | `policy.evidence.target-device.v1` |
| `gate.product.phase-8-cpp23-shipping-cross-target` | `phase.production-capability` | `fixture.product.cpp23-shipping-cross-target` | `requirement.product.cpp23-shipping-cross-target` | `target.headless.host; target.windows.editor; target.windows.desktop; target.android.mobile; target.apple.mobile` | `policy.product.same-candidate.v1` | `policy.evidence.target-device.v1` |
...
`gate.product.phase-8-cpp23-public-headers-cross-target`と`gate.product.phase-8-cpp23-shipping-cross-target`はCapability qualification専用bindingであり、`ProductPhaseRegistryV1.exit_gate_refs[]`には含めない。Phase bindingがPhase exitから未参照でも、`CapabilityTargetActivationBindingRegistryV1.qualification_gate_refs[]`からexactly one以上参照される上記2 IDだけはorphanではない。その他の未参照binding、またはPhase exitとCapability qualificationのどちらからも参照されないbindingを拒否する。この二GateによりHeader public surfaceとC++23 Shippingの5 Target計10行は`unavailable_no_gate`にならず、対応WP完了後に同一Candidate・全Target fresh Receiptを得た場合だけ`qualified`へ進める。C2 3Dの3行は§11.9の承認済み第二fixtureがまだないため意図的に`unavailable_no_gate`を維持する。
...
|---|---|---|---|---|---|---|---|---|
| `gate.product.qualify-cpp23-public-headers` | `mirakan.arch.cpp23-modules` | `all_of` | `gate.product.phase-0-headless-contract; gate.product.phase-2-windows-empty-scene; gate.product.phase-7-android-lifecycle; gate.product.phase-7-apple-lifecycle` | `[]` | `evidence.class.cpp23-required-feature-probes; evidence.class.cpp23-public-header-surface; evidence.class.cpp23-no-modules-boundary; evidence.class.toolchain-lock-exact` | `[]` | `blocked` | `{kind=permit_work_package_transition, work_package_id=wp.foundation.cpp23-public-headers, from_state=deferred, to_state=declared, transition_policy_ref=policy.product.wp.defer-release.v1}` |
| `gate.product.qualify-cpp23-shipping` | `mirakan.arch.cpp23-modules` | `all_of` | `gate.product.phase-0-headless-contract; gate.product.phase-2-windows-empty-scene; gate.product.phase-3-genreless-core; gate.product.phase-4-ai-genreless; gate.product.phase-7-android-lifecycle; gate.product.phase-7-apple-lifecycle` | `{work_package_id=wp.foundation.cpp23-public-headers, required_state=complete}` | `evidence.class.cpp23-formal-language-mode; evidence.class.cpp23-public-header-surface; evidence.class.cpp23-cross-target-release-readiness; evidence.class.package-install-offline-rollback-qualification` | `[]` | `blocked` | `{kind=permit_work_package_transition, work_package_id=wp.foundation.cpp23-shipping, from_state=deferred, to_state=declared, transition_policy_ref=policy.product.wp.defer-release.v1}` |
...
|---|---|---|---|---|
| `evidence.class.cpp23-required-feature-probes` | `mirakan.arch.cpp23-modules` | `policy.evidence.contract-ci.v1` | `target.headless.host; target.windows.editor; target.windows.desktop; target.android.mobile; target.apple.mobile` | required C++23 feature probe set、frontend identity、compiler artifact |
| `evidence.class.cpp23-public-header-surface` | `mirakan.arch.cpp23-modules` | `policy.evidence.contract-ci.v1` | `target.headless.host; target.windows.editor; target.windows.desktop; target.android.mobile; target.apple.mobile` | self-contained public Header compile／link／load fixture set |
| `evidence.class.cpp23-no-modules-boundary` | `mirakan.arch.cpp23-modules` | `policy.evidence.contract-ci.v1` | `target.headless.host; target.windows.editor; target.windows.desktop; target.android.mobile; target.apple.mobile` | Named Modules、header unit、BMI、`import std`のShipping surface混入0件 |
...
表の`scheduling_state`は`wp.foundation.cpp23-public-headers`、`wp.foundation.cpp23-shipping`、`wp.product.general-coverage-3d`、`wp.product.production-release-binding`の4件を`deferred`とし、それ以外を`declared`とする。計画書の存在を`ready`、`active`、`complete`へ読み替えず、deferred WPはPhase exitの完了条件へ含めない。
...
| `wp.platform.windows-package` | `phase.editor-runtime` | `mirakan.arch.platform-windows` | `target.windows.editor; target.windows.desktop` | `fallback.capability.unavailable` | `fixture.product.windows-empty-scene` | `capability.rendering.d3d12-backend` | `wp.rendering.d3d12-backend; wp.authoring.headless-core` | `declared` | `null` | `[]` | `null` |
| `wp.product.editor-runtime-windows` | `phase.editor-runtime` | `mirakan.arch.product-plan` | `target.windows.editor; target.windows.desktop` | `fallback.capability.unavailable` | `fixture.product.windows-empty-scene` | `capability.runtime.ecs-e4-game-system; capability.rendering.d3d12-backend; capability.platform.input-core; capability.platform.audio-core; capability.platform.ui-core; capability.platform.windows-package` | `wp.runtime.ecs-e4-game-system; wp.rendering.d3d12-backend; wp.platform.input-windows; wp.platform.audio-windows; wp.platform.ui-windows; wp.platform.windows-package` | `declared` | `null` | `[]` | `null` |
...
| `wp.foundation.cpp23-public-headers` | `phase.production-capability` | `mirakan.arch.cpp23-modules` | `target.headless.host; target.windows.editor; target.windows.desktop; target.android.mobile; target.apple.mobile` | `fallback.capability.unavailable` | `fixture.product.cpp23-public-headers-cross-target; fixture.product.headless-contract-smoke; fixture.product.windows-empty-scene; fixture.product.mobile-lifecycle` | `capability.foundation.cpp23-language-base` | `wp.foundation.cpp23-language-base; wp.platform.windows-package; wp.platform.android-package; wp.platform.apple-package` | `deferred` | `全Targetのself-contained Header、required C++23 feature、Tooling／ABI Receiptが未materializeのためqualificationを主張しない` | `gate.product.qualify-cpp23-public-headers` | `null` |
| `wp.foundation.cpp23-shipping` | `phase.production-capability` | `mirakan.arch.cpp23-modules` | `target.headless.host; target.windows.editor; target.windows.desktop; target.android.mobile; target.apple.mobile` | `fallback.capability.unavailable` | `fixture.product.cpp23-shipping-cross-target; fixture.product.headless-contract-smoke; fixture.product.windows-empty-scene; fixture.product.mobile-lifecycle` | `capability.foundation.cpp23-public-headers` | `wp.foundation.cpp23-public-headers` | `deferred` | `全Targetのrequired C++23 feature、Header public surface、Build／Tooling／ABI／Package／Release Receiptが未materializeのためRelease Activation不可` | `gate.product.qualify-cpp23-shipping` | `null` |
...
#### ECS Work Packageのinitial V1 owner mapping

Work Package表の`Owner`列はArchitecture Document Registry候補へ解決する`owner_document_id`である。`wp.runtime.ecs-e0`から`wp.runtime.ecs-e7-*`はinitial V1から[Runtime ECS](../04-runtime/entity-component-system.md)のexact document IDを直接参照し、SchedulingまたはGameplayを旧Runtime ECS authorityとして登録しない。表の存在はWork Package lifecycle、Capability activation、Owner Registryまたは実装のmaterializationを意味しない。

次表はProduct計画上の責務境界を示す閲覧用mappingである。
...
| `ecs-e4-game-system`、`ecs-e5-*-integration` | [Runtime ECS](../04-runtime/entity-component-system.md)＋[Gameplay programming model](../03-authoring/gameplay-programming-model.md) | System binding、Domain integration。ECS storageとGameplay authoringを分離する |
...
### Initial V1 projection: data-oriented ECS core

This subsection is a closed initial V1 proposal for the data-oriented ECS Product requirement. It is unmaterialized and does not change an operational Registry, Work Package lifecycle, Capability activation, or implementation state. It has no predecessor Product Definition, source／destination migration, Owner-reference migration, compatibility alias, or old reader. The exact Runtime ECS Owner／Definition refs and qualification evidence must be closed before any Registry or Capability can be materialized.
...
| `WorkPackageRegistryV1` | initial V1 rows bind `{kind=product_fixture, fixture_id=fixture.runtime.ecs-data-oriented-core}` in `provided_fixture_refs[]` of `wp.foundation.memory-pointers`, `wp.runtime.scheduling-core`, `wp.runtime.ecs-e0`, `wp.runtime.ecs-e1-storage`, and `wp.runtime.ecs-e2-query-mutation`; all ECS rows directly use the Runtime ECS Owner |
...
initial V1の`requirement.foundation.memory-pointer-contract` definition closureはexactly次の4 Typeである。
...
initial V1の`risk.product.memory-pointer-contract-drift.mitigation`は次のexact valueを持つ。
...
`wp.runtime.ecs-e0`、`wp.runtime.ecs-e1-storage`、`wp.runtime.ecs-e2-query-mutation`を含む全ECS Work Packageはinitial V1から`owner_document_id=mirakan.arch.runtime-entity-component-system`を直接持つ。`wp.foundation.memory-pointers`と`wp.runtime.scheduling-core`は各自のOwnerを維持し、旧Owner値またはOwner-reference migrationを定義しない。

initial V1 Requirementはfixture Componentのaccepted `RuntimeComponentLayoutPolicyV1` record、`ecs_chunk_soa_v1`を使う一つの`RuntimeArchetypeLayoutPlanV1`、query／lease／structural contract、全35 mandatory metric ID、全hard predicate、Target別`RuntimeEcsInitialAcceptanceThresholdSetV1`、同一Candidateに束縛した8192／16384／32768-byte characterization、Shipping AoS／sparse-set／object graph／general-heap fallback 0を要求する。
...
|---|---|---|---|---|---|---|---|---|---|---|
| `risk.product.ecs-data-oriented-regression` | `mirakan.arch.product-plan` | `wp.foundation.memory-pointers; wp.runtime.scheduling-core; wp.runtime.ecs-e0; wp.runtime.ecs-e1-storage; wp.runtime.ecs-e2-query-mutation` | missing layout policy、dual Shipping layout、hot callback allocation／fallback、unbounded archetype growth、missing campaign cell／metric, or wrong-Target Receipt substitution | `high` | `critical` | require the initial V1 Phase 0 Gate、complete campaign、hard predicates, and fresh Target-specific reruns | reject the affected ECS Work Package transition and dependent Runtime activation; retain the last qualified layout without an alternate Shipping fallback | `gate.product.phase-0-ecs-data-oriented-core` | `open` | `{kind=phase_gate, ref=gate.product.phase-0-ecs-data-oriented-core}` |
...
`wp.authoring.prequalified-source-packs`は初心者向けDefinition-first経路と、事前Qualification済みNative／Shader Packの選択だけを提供する。Phase 5の`wp.authoring.project-native-module`と`wp.rendering.project-shader`で新規AI生成Sourceを採用する場合は、各Ownerの独立したCode owner approval GateとSource／artifact／Receipt hash closureを必須とし、事前PackのReceiptを代用しない。`wp.rendering.project-shader`は`bounded_hlsl`と`typed_ir`を同一Source carrierまたは同一理解証拠へ統合せず、Project Shader ownerが定義するmode別closureを同じCandidate／Project revisionへ束ねる。`wp.product.project-source-activation`は両Source WPを集約するが、`wp.product.external-agent`とは依存もRequirementも共有せず、Proposal-only境界をSource生成の成功扱いにしない。
...
| `capability.foundation.cpp23-language-base` | C0 | `wp.foundation.cpp23-language-base` | Headless required; Windows Editor required; Windows required; Android required; Apple required | `fallback.capability.unavailable` |
| `capability.foundation.cpp23-public-headers` | C0 | `wp.foundation.cpp23-public-headers` | Headless required; Windows Editor required; Windows required; Android required; Apple required | `fallback.capability.unavailable` |
| `capability.foundation.cpp23-shipping` | C0 | `wp.foundation.cpp23-shipping` | Headless required; Windows Editor required; Windows required; Android required; Apple required | `fallback.capability.unavailable` |
...
| `capability.rendering.d3d12-backend` | C0 | `wp.rendering.d3d12-backend` | Windows Editor required; Windows required; Android excluded; Apple excluded | `fallback.capability.unavailable` |
...
| `risk.product.cpp23-cross-target-qualification` | `mirakan.arch.cpp23-modules` | `wp.foundation.cpp23-language-base; wp.foundation.cpp23-public-headers; wp.foundation.cpp23-shipping` | Named Modules、header unit、BMIまたは`import std`がShipping public surfaceへ混入する、required C++23 feature probeがTargetで失敗する、またはHeader／Tooling／ABI／Package／全Target Receiptのいずれかが成立しない | `high` | `critical` | initial V1をself-contained Header public surfaceへ固定し、required feature、禁止surface、Target別Toolchain／Configurationを同一Candidateのexact qualificationへ束縛する | C++23 Shipping qualificationとRelease Activationを拒否し、Named Modules、別Generatorまたはpreview Toolchainへfallbackしない | `gate.product.phase-0-headless-contract; gate.product.qualify-cpp23-public-headers; gate.product.qualify-cpp23-shipping` | `open` | `decision_gate:gate.product.qualify-cpp23-shipping` |
...
### 11.8 C++23 Header Shipping gate

initial V1の唯一のShipping public surfaceはself-contained Headerである。Named Modules、header units、BMI、`import std`はDevelopment、Test、candidate Packageを含むcurrent required universeへ入れず、fallback、移行元、Technology PreviewまたはRelease Activation入力として扱わない。`capability.rendering.d3d12-backend`も同じHeader public surfaceを消費し、Shipping Configurationのcandidate Package成功だけを公開Releaseの証拠にしない。

`wp.foundation.cpp23-public-headers`と`capability.foundation.cpp23-public-headers`は全TargetのHeader public surface qualification、`wp.foundation.cpp23-shipping`と`capability.foundation.cpp23-shipping`は全TargetのRelease有効化Evidenceを所有する。両Work Packageは初期`deferred`、両CapabilityのTarget行は初期`not_activated`であり、Planning Decision Gateが`satisfied`でもWork Packageの再schedulingだけを許可してActivationを自動昇格しない。

全Targetのrequired C++23 feature probe、self-contained Header、同じToolchain／Configuration、Build／Tooling／ABI／Package／Release Receiptのいずれかが欠ける場合はC++23 Shipping qualificationを拒否する。Visual Studio Generator由来BMI、MSVC 14.51の`/std:c++23preview`、Named Modulesまたは一部TargetのReceiptを代用にしない。
