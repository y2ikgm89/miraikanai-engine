...
本節のRegistry候補はinitial V1 Product Definitionを直接表す。Product Planが選ぶcompact 2D RPG Reference、3D technical Reference、optional Shooter fixture、Genre-neutral Core holdoutをpredecessor、source／destination Product migration、Pack ID renameまたはReceipt流用なしに別subjectとして閉じる。未定義ID、Availability、Receiptを本文から推測または生成せず、候補表の存在をmaterialized Registryまたはactive Capabilityと解釈しない。
