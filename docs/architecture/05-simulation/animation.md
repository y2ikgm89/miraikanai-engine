...
現行C1／C2はselected Motion Executorの`resolved_motion_schema_ref`へ適合するgeneration付きsnapshotだけを受け、AnimationがProvider-private state、native Body、Transform componentをqueryしない。`ragdoll`は`future.capability.vehicle-ragdoll-crowd-motion-warping`の予約語であり、同Future IDを採択する新しいActive Product Definition revisionがRagdoll固有Owner contract、exact Target binding、Authority、Save／Replay schema、positive／negative fixtureを同時に要求し、通常lifecycleで承認・Qualificationするまで、Animation input、Graph node、operationとして公開しない。昇格前のRagdoll要求は`capability_unavailable`で拒否し、pose、clock、event cursor、Project／Save／Replay stateを変更しない。将来昇格後もPhysicsは`SkeletonPose`へ直接writeせず、versioned Physics pose inputとAnimation poseの合成はAnimation ownerだけが行う。NavigationはAnimation parameter候補をcommand／snapshotで渡せるが、Graph stateを直接変更しない。
...
Animation presentation LODはpose update、presentation bone set、skinning mode、shadow poseを変更できるが、root motion、hitbox／weapon socket、foot contact、Gameplay event、authoritative boundsを低頻度presentation poseから取得しない。visibilityをGraph clock／transition／event cursor停止の入力にしない。共通LOD intent／resolutionは[LOD](../06-rendering/lod.md)、World dormancyは[World](../06-rendering/world.md)へ委譲する。Animation Source／Artifact／LOD Planはclass=`animation_presentation`のexact `LodDomainProfileRefV1={lod_class, subject_scope_ref, profile_id, profile_version, profile_content_hash}`を共有し、Profile IDだけ、Graph node index、geometry LOD indexからbindingを推測しない。

initial V1、C1およびC2のAnimation Source、Clip、Graph、Instance、Pose、event、root motion、Cooked AssetはMorph weight channelを持たない。Asset SourceにMorph default weightまたはanimated `weights` channelがあれば[Asset lifecycle](../03-authoring/asset-lifecycle.md)のunsupported-feature規則でImportを拒否し、channel drop、zero weight固定、base poseへのbakeまたはSkeleton trackへの変換を行わない。将来Morph Capabilityを採択する場合はSource Morph identity、weight track、time／interpolation、skinとの評価順、bounds／Render Snapshot、LOD／Virtual Geometry、fallback、Save／Replay、Target Qualificationを同じ新versionのclosureへ閉じる。

[Virtualized／Continuous Geometry](../06-rendering/virtualized-continuous-geometry.md)のtarget deformation compatibilityはAnimation Ownerのexact Skeleton／Skin binding、required presentation bone set、deformation kind、bounded deformation envelope、authoritative bounds relation、Target、constraint、fallback bindingを持つreceipt-free意味契約である。Morph bindingはinitial V1／C1／C2の集合に含めず、Future Morph Capabilityのend-to-end closureが承認された場合だけ新versionへ追加する。Target support stateとReceiptは同文書の`VirtualGeometryFeatureQualificationBindingV1`だけが所有する。static、skinning、将来のmorph target、world-position／spline／displacement deformationを一つの`animated` boolにせず、Feature RequirementごとにBindingの`unsupported | not_evaluated | provisional | qualified`を判定する。

target `VirtualGeometryDeformationCompatibilityRefV1`はcompatibility ID、positive version、content hashのexact refである。initial content hashはSkeleton／Skin／envelope／bounds、feature tuple、constraint、fallback bindingから計算し、Morph Fieldまたはempty placeholderを含めない。将来Morphを追加する場合は別versionでMorph identity／generationをhash preimageへ含める。Qualification Receipt／Binding／Activation Bindingをpreimageへ戻さない。

Geometry inner cutはpose sampling、Graph clock、transition、event cursor、root motion、hitbox／weapon socket、foot contact、authoritative boundsを変更しない。deformation envelope欠損／超過、Skin generation mismatch、必要bone／attribute欠損では該当virtual pathを使わずexact discrete skinned representationへ戻す。将来Morphを採択したversionではMorph generation mismatchも同じfail-closed条件へ加える。staticとして再解釈、bind pose、zero Morph weight、低頻度pose、event停止をfallbackにしない。current compatibility entry集合は`[]`である。
