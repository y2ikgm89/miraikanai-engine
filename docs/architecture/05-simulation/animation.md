...
[Virtualized／Continuous Geometry](../06-rendering/virtualized-continuous-geometry.md)のtarget deformation compatibilityはAnimation Ownerのexact Skeleton／Skin／Morph binding、required presentation bone set、deformation kind、bounded deformation envelope、authoritative bounds relation、Target、constraint、fallback bindingを持つreceipt-free意味契約である。Target support stateとReceiptは同文書の`VirtualGeometryFeatureQualificationBindingV1`だけが所有する。static、skinning、morph target、world-position／spline／displacement deformationを一つの`animated` boolにせず、Feature RequirementごとにBindingの`unsupported | not_evaluated | provisional | qualified`を判定する。

target `VirtualGeometryDeformationCompatibilityRefV1`はcompatibility ID、positive version、content hashのexact refである。content hashはSkeleton／Skin／Morph／envelope／bounds、feature tuple、constraint、fallback bindingから計算し、Qualification Receipt／Binding／Activation Bindingをpreimageへ戻さない。

Geometry inner cutはpose sampling、Graph clock、transition、event cursor、root motion、hitbox／weapon socket、foot contact、authoritative boundsを変更しない。deformation envelope欠損／超過、Skin／Morph generation mismatch、必要bone／attribute欠損では該当virtual pathを使わずexact discrete skinned representationへ戻す。staticとして再解釈、bind pose、低頻度pose、event停止をfallbackにしない。current compatibility entry集合は`[]`である。
