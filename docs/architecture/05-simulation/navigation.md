...
[LOD](../06-rendering/lod.md#7-simulation-lod境界)へNavigation behavior candidateを公開する場合は、Navigation Ownerがexact `SimulationLodCandidateDescriptorV1`、path／reachability／cost semantic guarantee、accepted request／result retention、wake condition、handoff、equivalence fixtureを定義・Qualificationする。現行targetは`full`だけで、低解像度Navmesh、遠距離agent停止、query間引き、stale cacheをSimulation LOD candidateとして登録していない。render visibility、HLOD cell、Camera distance、Quality、pressureからpath result、reachability、movement intent deliveryを変更しない。

[Virtualized／Continuous Geometry](../06-rendering/virtualized-continuous-geometry.md)のhierarchy、page residency、micro-cluster cut、pressure、fallbackをNav source、tile activation、obstacle、query priority、reachabilityまたはpath costへ入力しない。Navigation cookは[Collision](collision.md)のcanonical static geometryだけをsourceとし、virtual geometryの表示detail on／off、page loss、device recoveryでNavmesh version、query result、Movement Intentを変更しない。
