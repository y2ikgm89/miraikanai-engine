...
ShooterTargetProviderBindingOperationCommonInputV1
  ...
  authorization_binding: exact MutationAuthorizationBindingV1(risk_class=R2)
...
| `validator.genre.shooter.target_provider_binding.create_postcondition` | `type.operation.postcondition_evaluation_input@1` | `hash_mismatch; registry_invalid; receipt_binding_mismatch` |
...
| `validator.genre.shooter.target_provider_binding.update_postcondition` | `type.operation.postcondition_evaluation_input@1` | `hash_mismatch; registry_invalid; receipt_binding_mismatch` |
...
| `validator.genre.shooter.target_provider_binding.select_postcondition` | `type.operation.postcondition_evaluation_input@1` | `hash_mismatch; registry_invalid; receipt_binding_mismatch` |
...
Shooter Game Flow Scopeと`GameSystemSpecV1.runtime_scope_type_ref`はinitial V1から`scope.genre.shooter.game_flow.instance`を使用する。対応する旧`play_session` bytesまたはreader／writerはmaterializeされていないため、Runtime Scope migration contribution、offline Operation、alias、legacy fixtureをcurrent Packへ定義しない。末尾`.instance`欠落や同名の近似scopeをcurrent refへ変換せずrejectする。
