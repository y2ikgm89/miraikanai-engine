...
4章の共通C2と各SubsystemがC2と宣言するCapabilityは、次のread-only projectionへ決定論的に展開する。これはProduct-ownedの保存Registryではなく、`CapabilityRegistryV1`と`CapabilityTargetActivationV1`のcurrent bytesを入力とするQuery結果である。
...
C2CapabilityCoverageMatrixV1 // read-only projection
  ...
  rows[] := exact_join(row_keys, CapabilityRegistryV1, CapabilityTargetActivationV1):
    capability_id
    target_id
...
`target_product_tier=C2`である全Capabilityを例外なく入力集合とし、各`required | optional` bindingからexact `{capability_id, target_id}` keyを一意に導出する。`excluded` bindingは行を生成しない。各keyに対応する`CapabilityTargetActivationV1`行が欠落、重複、参照不能、またはC2 Capabilityに対して`row_keys`にないextra Activation keyが存在する場合はprojection全体とProduct labelを拒否し、行の省略、`not_activated`の合成、別Target行の流用を行わない。`state`、`candidate_ref`、`receipt_refs[]`、`evidence_freshness`はTarget Activation行からread-onlyで投影し、matrix固有のstate、Receipt、revision、entry table、`include_in_product_label`を保存しない。Owner、Target scope、fallbackはCapability Registry、defer理由、依存、再検討GateはWork Package Registryからだけ読み、Product labelとmatrix Receiptという二重正本を作らない。
...
| `future.capability.large-session-network-shooter` | `mirakan.arch.domain-pack-shooter` | `planning_only` | `capability.gameplay.shooter_core; capability.domain.shooter-2d; capability.domain.shooter-3d; capability.runtime.scheduling` | `product_scope; authority_model; data_model; threat_model; operations; qualification` | `headless_server; distributed_cluster; desktop; mobile` | `deterministic_simulation; network_fault; scale; target_device; security; operations` | `future.capability.headless-dedicated-server-session-transport-replication`と`future.capability.rollback-competitive-networking`が独立Decisionでactive Capabilityへ昇格した後、player count、server／region authority、replication／rollback、anti-cheat、failure recovery、運用SLOの承認Decisionとscale Receiptが揃う | `MVP-A; MVP-B; Technology Preview; multiplayer; large-session network shooter; high-player-count support` |
...
| `future.capability.first-person-shooter-profile` | `mirakan.arch.domain-pack-shooter` | `planning_only` | `capability.gameplay.shooter_core; capability.domain.shooter-3d; capability.camera.3d; capability.simulation.animation-3d; capability.platform.input-core; capability.platform.audio-core; capability.platform.ui-core` | `product_scope; data_model; target_program; qualification` | `desktop; mobile; console` | `contract; deterministic_simulation; target_device; content_quality` | first-person Camera／aim／weapon presentation、authoritative 3D Animation、Input／Audio／UI／comfort／Accessibility、2D／TPSと意味共有するProfile、独立fixture、Target別performanceを閉じ、Console候補ではTarget program Decisionも成立する | `MVP-A; MVP-B; Technology Preview; FPS support; first-person shooter profile` |
...
| `future.capability.aaa-photoreal-rendering` | `mirakan.arch.rendering-environment-surfaces` | `planning_only` | `capability.world.3d; capability.camera.3d; capability.environment.core; capability.render.material.realistic_advanced; capability.vfx.system` | `product_scope; data_model; provider_selection; licensing; target_program; qualification` | `desktop; console` | `contract; content_quality; scale; target_device; operations` | general 3D productionのTarget別production Receipt後に、photoreal quality rubric、content pipeline、lighting／material／environment／VFX fidelity、hardware budget、意味同等fallbackの承認Decisionとblind comparison Receiptが揃う | `MVP-A; MVP-B; Technology Preview; AAA visual parity; photoreal rendering guarantee` |
...
ProductRiskRegistryV1
  entries[]: risk_id, owner_document_id, affected_work_package_refs[], trigger,
             likelihood, impact, mitigation, contingency, evidence_refs[], state,
...
ProductDecisionGateRegistryV1
  entries[]: gate_id, owner_document_id, predicate, state, evidence_refs[],
             on_satisfied_action
```

`scheduling_state`のclosed値は`declared | ready | active | blocked | deferred | complete`である。`defer_reason`と`blocked_reason_ref`は常にFieldを持ち、非該当時は`null`とする。`reconsideration_gate_refs[]`は常に存在し、非deferred時は空配列である。`scheduling_state=deferred`ではnon-empty `defer_reason`と1件以上の`reconsideration_gate_refs[]`、`blocked`ではnon-null `blocked_reason_ref`を必須とする。他stateでこれらを設定した行を拒否する。`reconsideration_gate_refs[]`は`PhaseFixtureBindingRegistryV1.gate_id`または`ProductDecisionGateRegistryV1.gate_id`のexact IDだけを受理する。
...
Activationの唯一の保存状態正本は`CapabilityTargetActivationV1`である。`CapabilityRegistryV1`の各`target_bindings[]`について、scopeが`required`または`optional`の`{capability_id, target_id}`行を初回生成し、`state=not_activated`、`candidate_ref=null`、`receipt_refs=[]`、`evidence_freshness=expired`とする。scopeが`excluded`のTargetにはActivation行を生成せず、選択を拒否する。Capability全体の表示状態はrequired Target行の最小stateから毎回導出するread-only projectionであり、Registry、C2 matrix、Capability表、Product labelへ保存またはwrite-backしない。
...
`capability.domain.shooter-2d`と`capability.domain.shooter-3d`はWindows／Android／Appleに別々の`CapabilityTargetActivationV1`行を持つ。Phase 3の`gate.product.phase-3-manual-2d`はShooter 2DのWindows行だけ、Phase 6の`gate.product.phase-6-first-playable-3d`はShooter 3DのWindows行だけを昇格候補にする。Phase 7の四つの`*-runtime-2d`／`*-runtime-3d` Gateは対応dimensionとAndroidまたはAppleの一行だけを昇格候補にし、Windows、反対側Mobile Target、別dimensionのReceiptを流用しない。Phase 8 genre GateはC2 coverageを評価するだけでC1 Shooter行を遡及昇格しない。

`ProductDecisionGateRegistryV1.state`のclosed値は`blocked | open | satisfied | retired`である。Decision GateはPhase exit Receiptではなく、未登録logical IDを作らずにplanning-only条件を追跡する。`state=satisfied`でも`on_satisfied_action`のChangeSetを自動適用せず、Owner approvalと全Registry validationを別途必須とする。
...
| `fixture.product.shooter-2d` | `mirakan.arch.domain-pack-shooter` | `requirement.target.mobile-runtime-2d; requirement.product.manual-first-playable-2d; requirement.product.ai-authoring-mvp-a; requirement.product.authoring-roundtrip; requirement.product.title-to-result; requirement.product.save-load-replay; requirement.product.mvp-completion; requirement.product.project-source-activation; requirement.product.c2-2d-coverage` | `target.windows.editor; target.windows.desktop; target.android.mobile; target.apple.mobile` | 300 |
...
| `fixture.product.shooter-arena-3d` | `mirakan.arch.domain-pack-shooter` | `requirement.target.mobile-runtime-3d; requirement.product.first-playable-3d; requirement.product.save-load-replay; requirement.product.c2-3d-coverage` | `target.windows.desktop; target.android.mobile; target.apple.mobile` | 300 |
...
| `gate.product.phase-3-manual-2d` | `phase.manual-2d` | `fixture.product.shooter-2d` | `requirement.product.manual-first-playable-2d` | `target.windows.desktop` | `policy.product.same-candidate.v1` | `policy.evidence.target-device.v1` |
| `gate.product.phase-4-ai-mvp-a` | `phase.ai-authoring-mvp-a` | `fixture.product.shooter-2d` | `requirement.product.ai-authoring-mvp-a` | `target.windows.editor; target.windows.desktop` | `policy.product.same-candidate.v1` | `policy.evidence.target-device.v1` |
...
| `gate.product.phase-6-first-playable-3d` | `phase.manual-3d-mvp-b` | `fixture.product.shooter-arena-3d` | `requirement.product.first-playable-3d` | `target.windows.desktop` | `policy.product.same-candidate.v1` | `policy.evidence.target-device.v1` |
...
| gate_id | Owner | predicate | state | evidence refs | on satisfied action |
|---|---|---|---|---|---|
| `gate.product.reconsider-cpp23-cx2-cutover` | `mirakan.arch.cpp23-modules` | CMakeの`import std`がExperimental tokenなしでWindows／AppleのNinja Multi-ConfigとAndroidのSingle-Config Ninjaに対応し、全TargetのCX0↔CX2 API／ABI／Unit／Save／Replay／Package／Tooling比較Receipt、module DAG、Header削除計画、exact Toolchain lockが同一Candidateへ閉じ、duplicate、missing ref、Target closure、DAG validationが0 errorになる | `blocked` | `gate.product.phase-0-headless-contract; gate.product.phase-2-windows-empty-scene; gate.product.phase-7-android-lifecycle; gate.product.phase-7-apple-lifecycle` | `wp.foundation.cpp23-cx2-cutover`を`deferred`から`declared`へ遷移可能にする。CapabilityTargetActivation行の作成または昇格は行わない |
| `gate.product.reconsider-cpp23-cx3-shipping` | `mirakan.arch.cpp23-modules` | CX2 cutover Work Packageがcompleteで、全Targetのfresh Cutover Receipt、正式C++23 language mode、MSVC stable正式`/std:c++23`、Experimental tokenなしのCMake、Ninja系C++ archive、clean Package／install／offline run／rollback／Release Receiptが同一Candidateへ閉じ、duplicate、missing ref、Target closure、DAG validationが0 errorになる | `blocked` | `gate.product.phase-0-headless-contract; gate.product.phase-2-windows-empty-scene; gate.product.phase-7-android-lifecycle; gate.product.phase-7-apple-lifecycle; gate.product.phase-8-c2-shooter-2d; gate.product.phase-8-c2-platformer-2d; gate.product.phase-8-c2-puzzle-dialogue-2d` | `wp.foundation.cpp23-cx3-shipping`を`deferred`から`declared`へ遷移可能にする。CapabilityTargetActivation行の作成または昇格は行わない |
| `gate.product.reconsider-c2-3d` | `mirakan.arch.product-plan` | 第二の非Shooter 3D Fixture、`requirement.product.c2-3d-coverage` binding、Genre／Rendering／UI provider WP、Owner、Windows／Android／Apple Target bindingが一つの承認済みProduct Decisionでactive Registryへ追加され、duplicate、missing ref、Target closure、DAG validationが0 errorになる | `blocked` | `gate.product.phase-6-first-playable-3d` | `wp.product.general-coverage-3d`を`deferred`から`declared`へ遷移可能にする。CapabilityTargetActivation行の作成または昇格は行わない |

### 11.5 Work Package registry

表の`scheduling_state`は`wp.foundation.cpp23-cx2-cutover`、`wp.foundation.cpp23-cx3-shipping`、`wp.product.general-coverage-3d`の3件を`deferred`とし、それ以外を`declared`とする。計画書の存在を`ready`、`active`、`complete`へ読み替えず、deferred WPはPhase exitの完了条件へ含めない。
...
| `wp.domain.shooter-2d` | `phase.manual-2d` | `mirakan.arch.domain-pack-shooter` | `target.windows.desktop; target.android.mobile; target.apple.mobile` | `fallback.capability.unavailable` | `fixture.product.shooter-2d` | `capability.gameplay.shooter_core; capability.runtime.ecs-e5-2d-integration; capability.camera.2d` | `wp.domain.shooter-core; wp.runtime.ecs-e5-2d-integration; wp.rendering.camera-2d` | `declared` | `null` | `[]` | `null` |
...
| `wp.domain.shooter-3d` | `phase.manual-3d-mvp-b` | `mirakan.arch.domain-pack-shooter` | `target.windows.desktop; target.android.mobile; target.apple.mobile` | `fallback.capability.unavailable` | `fixture.product.shooter-arena-3d` | `capability.gameplay.shooter_core; capability.runtime.ecs-e5-3d-integration; capability.camera.3d` | `wp.domain.shooter-core; wp.runtime.ecs-e5-3d-integration; wp.rendering.camera-3d` | `declared` | `null` | `[]` | `null` |
...
| `capability.gameplay.shooter_core` | C1 | `wp.domain.shooter-core` | Windows required; Android required; Apple required | `fallback.capability.unavailable` |
...
`required_capability_refs[]`はconsumer WPの各`target_refs[]`で実行時に必要なCapabilityだけを列挙する。各edgeについてCapability表の同Target scopeが`required`または`optional`でなければ拒否し、`excluded`をrequired edgeに使わない。headless authoring、build host、Code owner、Phase順序などcross-targetのorderingだけを表す関係は`requires_work_package_refs[]`へ置き、Target availabilityの証拠にしない。`defer_reason`、依存、`reconsideration_gate_refs[]`はWork Packageだけが所有し、Capability表または`CapabilityTargetActivationV1`へ複写しない。

### 11.7 Product risk registry

`likelihood`のclosed値は`low | medium | high | unknown`、`impact`は`moderate | major | critical`、`state`は`open | monitoring | mitigated | accepted | closed`である。`revisit_gate_or_date`は`kind=phase_gate`なら登録済みPhase Fixture Gateへの`ref`、`kind=decision_gate`なら登録済みProduct Decision Gateへの`ref`、`kind=date`なら根拠のあるISO 8601 `YYYY-MM-DD`の`value`だけを受理し、kind不一致Field、unknown kind、裸の文字列を拒否する。Markdown表は厳密な略記`phase_gate:<exact-id>`、`decision_gate:<exact-id>`、`date:<YYYY-MM-DD>`だけを許し、Registry生成時に上記discriminated unionへ展開する。本Revisionは未確定日程を作らず、全行をexact gateで再評価する。`evidence_refs[]`も登録済みgate IDだけを受理する。

| risk_id | Owner | affected work packages | trigger | likelihood | impact | mitigation | contingency | evidence refs | state | revisit gate or date |
|---|---|---|---|---|---|---|---|---|---|---|
...
`capability.product.general_production_3d`のTarget別保存状態は`CapabilityTargetActivationV1`だけが所有する。Decision Gateの`satisfied`、Work Packageの再scheduling、Shooter 3D ReceiptのいずれもTarget行を自動昇格せず、第二fixtureを含む新しいPhase bindingのfresh Receiptが揃うまでProduct labelを`3D First Playable`に限定する。
