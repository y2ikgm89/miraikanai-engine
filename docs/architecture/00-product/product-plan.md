...
`owner_work_package_ref`、`validator_refs[]`、`fixture_refs[]`、`target_refs[]`、`fallback_ref`、`qualification_receipt_refs[]`は必須である。一つでも空、参照不能、失効、対象Candidateとhash不一致ならProduct labelへのread-only projectionから除外し、Production表示を拒否する。Work Packageから外したCapabilityもMatrixから削除せず、owner Work Packageの`scheduling_state=deferred`とdefer理由、依存、再検討GateをWork Package側だけに保持する。C2 matrixはActivation stateまたは`include_in_product_label`を保存せず、Product labelは`CapabilityTargetActivationV1`のrequired Target行がすべて`production`である場合だけ導出するread-only projectionである。
...
| 7 | `phase.mobile` | `requirement.target.android-package; requirement.target.apple-package` | `wp.rendering.vulkan-backend; wp.rendering.metal-backend; wp.platform.mobile-offline; wp.platform.mobile-io-ui-android; wp.platform.mobile-io-ui-apple; wp.runtime.ecs-e7-android-2d; wp.runtime.ecs-e7-apple-2d; wp.runtime.ecs-e7-android-3d; wp.runtime.ecs-e7-apple-3d; wp.platform.android-package; wp.platform.apple-package` | `gate.product.phase-7-android-lifecycle; gate.product.phase-7-apple-lifecycle` |
...
| `gate.product.phase-5-project-source-activation` | `phase.external-agent` | `fixture.product.shooter-2d` | `requirement.product.project-source-activation` | `target.windows.editor; target.windows.desktop` | `policy.product.same-candidate.v1` | `policy.evidence.contract-ci.v1` |
...
| `capability.domain.shooter-2d` | C1 | `wp.domain.shooter-2d` | Windows required; Android excluded; Apple excluded | `fallback.capability.unavailable` |
...
| `capability.domain.shooter-3d` | C1 | `wp.domain.shooter-3d` | Windows required; Android excluded; Apple excluded | `fallback.capability.unavailable` |
...
| `capability.product.runtime-generation-boundary` | C0 boundary | `wp.product.runtime-generation` | Windows required; Android required; Apple required | `fallback.capability.unavailable` |
...
| `risk.product.c1-performance-calibration` | `mirakan.arch.runtime-performance-capacity` | `wp.gameplay.core-c1; wp.domain.shooter-2d; wp.domain.shooter-3d` | C1 fixtureのframe、memory、load、soak基準が実測Target profileでcalibrateされていない | `unknown` | `major` | Phase 3／6 Candidateで同一入力のbaselineを測定し、推測値をReceiptへ置換する | C1表示を対象Targetで保留し、機能削減を勝手に行わない | `gate.product.phase-3-manual-2d; gate.product.phase-6-first-playable-3d` | `open` | `phase_gate:gate.product.phase-6-first-playable-3d` |
| `risk.product.mobile-backend-closure` | `mirakan.arch.rendering-render-graph` | `wp.rendering.vulkan-backend; wp.rendering.metal-backend; wp.platform.mobile-io-ui-android; wp.platform.mobile-io-ui-apple; wp.runtime.ecs-e7-android-2d; wp.runtime.ecs-e7-apple-2d; wp.runtime.ecs-e7-android-3d; wp.runtime.ecs-e7-apple-3d; wp.platform.android-package; wp.platform.apple-package` | Mobile backend、I/O／UI、2D／3D ECS qualificationのいずれかが対象Targetで未合格またはWindows Receiptを流用する | `high` | `critical` | Android／Apple別WPとCapabilityTargetActivation行を使い、Windows-excluded edgeを0件にする | 該当Mobile TargetのpackageとShipping labelを拒否する | `gate.product.phase-7-android-lifecycle; gate.product.phase-7-apple-lifecycle` | `open` | `phase_gate:gate.product.phase-7-apple-lifecycle` |
