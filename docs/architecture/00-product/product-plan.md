...
このRegistryによりTier、WP existence、provided fixtureだけからActivationを推測しない。optional rowも同じEvidence／execution mappingを持つがProduct aggregateには含めない。current 24 planning family／191候補は全て`not_activated`であり、上表のpositive十CapabilityはWP状態にかかわらず現時点で`qualified`になれない。
