...
| 8 | `phase.production-capability` | `requirement.product.c2-2d-coverage` | `wp.domain.platformer; wp.domain.puzzle-dialogue; wp.rendering.environment-c2; wp.rendering.vfx-c2; wp.rendering.material-realistic; wp.rendering.material-toon; wp.ui.native-widget; wp.product.general-coverage-2d; wp.product.general-coverage-3d` | `gate.product.phase-8-c2-shooter-2d; gate.product.phase-8-c2-platformer-2d; gate.product.phase-8-c2-puzzle-dialogue-2d` |
...
表の`scheduling_state`は`wp.product.general-coverage-3d=deferred`を除きすべて`declared`である。計画書の存在を`ready`、`active`、`complete`へ読み替えず、deferred WPはPhase exitの完了条件へ含めない。
...
| `risk.product.cpp23-modules-cmake-constraint` | `mirakan.arch.cpp23-modules` | `wp.foundation.cpp23-cx0` | CX0 CMake／compilerがModules dependency scan、BMI順序、または`.ixx`禁止境界を満たさない | `medium` | `major` | CX0はheader／source baselineで固定し、Modules採用をToolchain Receiptで明示判定する | CX0をnon-module構成へ留め、後続WPの開始を止める | `gate.product.phase-0-headless-contract` | `open` | `phase_gate:gate.product.phase-0-headless-contract` |
