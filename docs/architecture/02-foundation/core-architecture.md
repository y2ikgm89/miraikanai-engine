...
- AI production control planeはEditorHost、GameHost、WorkerHostおよびtrusted Project Gatewayと同じmutable memory trust domainに置かず、Coreが管理するisolated Tooling Host process groupからtyped IPCを使う。Engine memoryやProject fileを直接変更しない。現在契約参照またはdispatchできる型付きIPC Operationはexact 0件である。[Executable contracts](executable-contracts.md)のtarget-complete十候補、reserved 207 ID、legacy migrationはcurrent MCD／IPC／Provider／MCPへ投影しない。Operation名、文書内templateまたは未Activation候補からdispatchを推測せず、将来materializeしたexact Operationだけを[AI Security／Approval](../01-governance/ai-security-approval.md)のAuthorizationで許可する。

[AI Production Orchestration](../03-authoring/ai-production-orchestration.md)のClient Surface Adapter、Orchestrator、Context Compiler、Route Resolver、Workflow Executor、first-party Agent Hostはlogical roleであり、Coreはそれらをbinary名、Directory名または一role一processへ固定しない。EditorHostはoptional AI Consoleとread projectionだけを持ち、GameHostはAuthoring AI／Providerへ接続しない。Provider Adapter、local inference、external Host BrokerはAI Security Profileが要求する別process／sandbox境界を維持し、Engine Automation ProjectionとProject Gatewayだけをtrusted Engine sideへ置く。logical roleを同一Tooling Hostへco-locateする場合もCredential、filesystem、network、Signer、Project writeのauthorityをunionしない。

AI Tooling Hostの停止、失効、upgrade、crashまたは不在はmanual Editor／CLI／SDKのcanonical Operationを無効化しない。AI-specific Run／Workflow／Context Storeが破損してもProject Source、current revision、Build／Test／Package／launch authorityを再構成できなければならず、AI stateをProject stateの唯一のcopyにしない。
