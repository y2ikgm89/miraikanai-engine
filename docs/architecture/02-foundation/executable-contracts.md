...
| `mcd_version=1; kind=operation; id=operation.project.runtime_entry.migrate_root_scene; version=1; status=active; title=Migrate Root Scene to Runtime Entry; description=Atomically create or update World Selector Policy and Runtime Entry Documents from one legacy root Scene closure; owners=[owner.core.project_state]; requirement_refs=[]; rationale_refs=[mirakan.arch.project-state#312-runtime-entryのclosed-operation-catalog]; since_contract_set=1; supersedes=[]; tags=[authoring,migration,runtime_entry]` | `operation_kind=command; input_type={type.project.runtime_entry.migrate_root_scene_input,1,contract_set_hash}; output_type={type.project.runtime_entry.mutation_result,1,contract_set_hash}; authority={service.authoring_command_gateway,1,content_hash}; risk_class=R3; side_effects=[authoring]; idempotency=idempotent_with_key; transaction=authoring_changeset; preconditions=[{policy.operation.project.runtime_entry.migrate_root_scene.precondition,1,contract_set_hash}]; postconditions=[{policy.operation.project.runtime_entry.migrate_root_scene.postcondition,1,contract_set_hash}]; errors=[{MIRAKAN-CONFLICT-REVISION_MISMATCH,1,content_hash},{MIRAKAN-AUTHORIZATION-DENIED,1,content_hash},{MIRAKAN-APPROVAL-REQUIRED,1,content_hash},{MIRAKAN-AUTHORING-LOCK_CONFLICT,1,content_hash},{MIRAKAN-MCD-OPERATION-PREDICATE_INVALID,1,content_hash},{MIRAKAN-PROJECT-RUNTIME_ENTRY_DANGLING_REFERENCE,1,content_hash},{MIRAKAN-PROJECT-RUNTIME_ENTRY_DEFAULT_AMBIGUOUS,1,content_hash},{MIRAKAN-PROJECT-RUNTIME_ENTRY_DOCUMENT_HASH_MISMATCH,1,content_hash},{MIRAKAN-PROJECT-RUNTIME_ENTRY_IDENTITY_MISMATCH,1,content_hash},{MIRAKAN-PROJECT-RUNTIME_ENTRY_MIGRATION_REQUIRED,1,content_hash},{MIRAKAN-PROJECT-RUNTIME_ENTRY_SCHEMA_MISMATCH,1,content_hash},{MIRAKAN-PROJECT-RUNTIME_ENTRY_TARGET_UNRESOLVED,1,content_hash},{MIRAKAN-OPERATION-TIMEOUT,1,content_hash}]; timeout_ms=120000; rate_limit_policy={policy.authoring.runtime_entry.rate_limit,1,contract_set_hash}; audit_level=full_redacted; provider_exposure=mcp_proposal; receipt_type={type.project.runtime_entry.mutation_receipt,1,contract_set_hash}` |
| `mcd_version=1; kind=operation; id=operation.runtime_scope.migrate_game_system_v1_to_v2; version=1; status=active; title=Migrate Game System Runtime Scope; description=Atomically migrate legacy Game System scope and auxiliary record references to the current typed schema; owners=[owner.core.gameplay_programming_model]; requirement_refs=[]; rationale_refs=[mirakan.arch.gameplay-programming-model#312-scope依存recordとoffline-migration]; since_contract_set=1; supersedes=[]; tags=[authoring,migration,runtime_scope]` | `operation_kind=job; input_type={type.runtime_scope.game_system_v1_migration_input,1,contract_set_hash}; output_type={type.runtime_scope.game_system_v1_migration_result,1,contract_set_hash}; authority={service.offline_project_migrator,1,content_hash}; risk_class=R3; side_effects=[authoring]; idempotency=idempotent_with_key; transaction=authoring_changeset; preconditions=[{policy.operation.runtime_scope.migrate_game_system_v1_to_v2.precondition,1,contract_set_hash}]; postconditions=[{policy.operation.runtime_scope.migrate_game_system_v1_to_v2.postcondition,1,contract_set_hash}]; errors=[{MIRAKAN-CONFLICT-REVISION_MISMATCH,1,content_hash},{MIRAKAN-AUTHORIZATION-DENIED,1,content_hash},{MIRAKAN-APPROVAL-REQUIRED,1,content_hash},{MIRAKAN-AUTHORING-LOCK_CONFLICT,1,content_hash},{MIRAKAN-MCD-OPERATION-PREDICATE_INVALID,1,content_hash},{MIRAKAN-RUNTIME-SCOPE-CATALOG_INVALID,1,content_hash},{MIRAKAN-RUNTIME-SCOPE-MIGRATION_CONFLICT,1,content_hash},{MIRAKAN-RUNTIME-SCOPE-OWNER_UNAVAILABLE,1,content_hash},{MIRAKAN-RUNTIME-SCOPE-VERSION_HASH_MISMATCH,1,content_hash},{MIRAKAN-OPERATION-TIMEOUT,1,content_hash}]; timeout_ms=120000; rate_limit_policy={policy.authoring.runtime_scope_migration.rate_limit,1,contract_set_hash}; audit_level=full_redacted; provider_exposure=mcp_proposal; receipt_type={type.runtime_scope.migration_receipt,1,contract_set_hash}` |

pre／postconditionは新kindではなく、次の16件のactive `policy` MCDである。全recordは`mcd_version=1`、`kind=policy`、`version=1`、`status=active`、`requirement_refs=[]`、`since_contract_set=1`、`supersedes=[]`、`tags=[operation_predicate,pure]`、`evaluation_mode=pure`、`side_effects=[]`、`result_type={id=type.operation.predicate_result,version=1,contract_set_hash}`を持ち、表が各recordの残り全値を明示する。preconditionのinput typeはexact Operation input ref／hashとread-only before snapshot refs／hashesを持つ`type.operation.precondition_evaluation_input` version 1、postconditionはrequest hash、exact Operation output ref／hash、before／after snapshot refs／hashes、Receipt refs／hashesを持つ`type.operation.postcondition_evaluation_input` version 1である。どちらもcanonical immutable valueだけを入力にし、Project／Registry／clock／networkを評価中にqueryしない。
...
| `policy.operation.project.runtime_entry.create.postcondition` | Create Runtime Entry Postcondition／identity三者一致のDocument一件とrevision増分を検証 | `[owner.core.project_state]`／`[mirakan.arch.project-state#312-runtime-entryのclosed-operation-catalog]`／`{type.operation.postcondition_evaluation_input,1,contract_set_hash}` |
...
| `policy.operation.project.runtime_entry.update.postcondition` | Update Runtime Entry Postcondition／同ID新revisionとconsumer invalidationを検証 | `[owner.core.project_state]`／`[mirakan.arch.project-state#312-runtime-entryのclosed-operation-catalog]`／`{type.operation.postcondition_evaluation_input,1,contract_set_hash}` |
...
| `policy.operation.project.runtime_target_selector.create.postcondition` | Create Target Selector Postcondition／selector identity、hash、default coverageを検証 | `[owner.core.project_state]`／`[mirakan.arch.project-state#312-runtime-entryのclosed-operation-catalog]`／`{type.operation.postcondition_evaluation_input,1,contract_set_hash}` |
...
| `policy.operation.project.runtime_target_selector.update.postcondition` | Update Target Selector Postcondition／同ID新revisionと全consumer coverageを検証 | `[owner.core.project_state]`／`[mirakan.arch.project-state#312-runtime-entryのclosed-operation-catalog]`／`{type.operation.postcondition_evaluation_input,1,contract_set_hash}` |
| `policy.operation.project.runtime_entry_activation_policy.create.precondition` | Create Activation Policy Precondition／closed policy draft、hash、Project revisionを検証 | `[owner.core.project_state]`／`[mirakan.arch.project-state#312-runtime-entryのclosed-operation-catalog]`／`{type.operation.precondition_evaluation_input,1,contract_set_hash}` |
| `policy.operation.project.runtime_entry_activation_policy.create.postcondition` | Create Activation Policy Postcondition／policy identity三者一致と新revisionを検証 | `[owner.core.project_state]`／`[mirakan.arch.project-state#312-runtime-entryのclosed-operation-catalog]`／`{type.operation.postcondition_evaluation_input,1,contract_set_hash}` |
...
| `policy.operation.project.runtime_entry_activation_policy.update.postcondition` | Update Activation Policy Postcondition／同ID新revisionとconsumer invalidationを検証 | `[owner.core.project_state]`／`[mirakan.arch.project-state#312-runtime-entryのclosed-operation-catalog]`／`{type.operation.postcondition_evaluation_input,1,contract_set_hash}` |
...
| `policy.operation.project.runtime_entry.migrate_root_scene.postcondition` | Root Scene Migration Postcondition／World、selector、policy、entryの四Document atomic resultを検証 | `[owner.core.project_state]`／`[mirakan.arch.project-state#312-runtime-entryのclosed-operation-catalog]`／`{type.operation.postcondition_evaluation_input,1,contract_set_hash}` |
| `policy.operation.runtime_scope.migrate_game_system_v1_to_v2.precondition` | Runtime Scope Migration Precondition／legacy Source、Catalog、七dependency、identity mapping、Approvalを検証 | `[owner.core.gameplay_programming_model]`／`[mirakan.arch.gameplay-programming-model#312-scope依存recordとoffline-migration]`／`{type.operation.precondition_evaluation_input,1,contract_set_hash}` |
| `policy.operation.runtime_scope.migrate_game_system_v1_to_v2.postcondition` | Runtime Scope Migration Postcondition／typed Game System revision、Source／Save／Replay mapping、Receiptを検証 | `[owner.core.gameplay_programming_model]`／`[mirakan.arch.gameplay-programming-model#312-scope依存recordとoffline-migration]`／`{type.operation.postcondition_evaluation_input,1,contract_set_hash}` |

`request_hash`は全八inputで`SHA-256(ASCII domain separator "MIRAKAN_OPERATION_REQUEST_V1" || request_hash Fieldを除外したexact inputのMCD canonical bytes)`とする。Operation ref、Project ref、policy refs、authorization／approval refsもhash入力であり、field順、omission rule、Contract set hashが変われば別requestである。
...
AffectedDocumentMutationV1
  ...
  document_kind: world | runtime_target_selector |
                 runtime_entry_activation_policy | runtime_entry
  before_document_ref: exact DocumentRef | omitted
  before_payload_semantic_hash: SHA-256 | omitted
  after_document_ref: exact DocumentRef
  after_payload_semantic_hash: SHA-256
```

`created`はbefore二Fieldをcanonical omissionし、`updated`はbefore二Fieldを必須にしてbefore／after Stable IDを一致させる。通常create／updateは対象一件、root Scene migrationは順にWorld、selector、policy、entryの四件を必須にし、各要素がcreatedかupdatedかを個別に記録する。null、zero hash、単数`affected_document`へ四件を圧縮しない。

Operation Registry compilerは八Operation ID、16 policy ID、全input／output／receipt／rate-limit ref、authority、Diagnostic refをexact Contract set／Registryへ解決する。pre／postcondition refがmissing、`kind!=policy`、version／Contract set hash stale、policyが`evaluation_mode!=pure`、またはside effect非空なら`MIRAKAN-MCD-OPERATION-PREDICATE_INVALID`でRegistry全体を拒否する。fixtureは八Operationのmeta-schema compileとProject ownerとのset equalityに加え、wrong-kind、missing、stale version、stale Contract set hash、impure policyを一原因ずつ拒否する。
