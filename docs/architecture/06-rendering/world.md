...
`SpatialTransitionRequestV1`は`request_id`、source Space instance ref、exact `SpatialTransitionDestinationRefV1`、requesting system ref、`requested_advance_sequence`、typed `transfer_subject_refs[]`、precondition snapshot hash、transition policy ref／hashを持つ。Edge／Space／AnchorをRequestへ複製しない。表示名から対象を再解決せず、stale World／Topology／Edge／Space／Anchor、hash mismatch、inactive sourceをprefetch前に拒否する。
