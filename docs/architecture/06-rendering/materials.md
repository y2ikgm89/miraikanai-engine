...
本書はMaterial、Shader、Visual Styleとlight-transport response inputの意味だけを所有する。Render pass／resource／queue／AAは[Render Graph](render-graph.md)、physical light semanticsは[Lighting](lighting.md)、GI／reflection／advanced shadow／reference transportのchannel／Technique／fallbackは[Advanced Light Transport](advanced-light-transport.md)、volume／effect compositionは[Post Processing](post-processing.md)、representation selectionは[LOD](lod.md)を参照する。ALTはMaterial response summaryを消費するがBSDF、Shading Model、parameter、Project Shader Sourceを書き換えない。
...
glTF `TANGENT`はXYZ normalized、Wをexact `1.0`または`-1.0`のhandednessとして保持し、bitangentを`cross(normal, tangent.xyz) * tangent.w`で得る。validなprovided `TANGENT`を優先し、欠落かつnormal textureがtangent basisを必要とする場合だけ、そのnormal textureが参照するexact `TEXCOORD_n`、position、normalから[Toolchain／Dependencies §9.1](../02-foundation/toolchain-dependencies.md#gltf-tangent-dependency-state)で選定したMikkTSpaceにより生成する。生成結果はtriangle corner単位であり、既存vertex indexへ平均／上書きせず、handedness、UV／normal discontinuity、tangent seamをcanonical vertex split条件に含める。Importerごとの別algorithm、filename、DCC名、cross-productだけの近似、normal map無効化またはruntime生成へfallbackしない。normal欠落時はglTF規則に従うflat normal生成とprovided tangent無効化を同じConversion Reportへ記録する。invalid／non-finite／生成失敗はImport failureである。Tangent／normal生成結果、color encoding、IBL prefilter／BRDF integration artifact、Target Profileの一つでも異なる場合は別PBR input closureとして再Qualificationする。
...
glTF Parser／Importer dependencyの選定状態、exact version／commit、artifact hash、license、取得元は[Toolchain／Dependencies §9.1](../02-foundation/toolchain-dependencies.md#gltf-tangent-dependency-state)だけが所有する。current stateはArchitecture target selected／dependency materialization absentであり、`toolchain.lock.json`、SBOM、Adapter、Fixture、Receipt、Qualificationも不在なのでglTF Source analysis／Import／Cook／promotionを開始しない。Source transport、sandbox Job、typed IR、Conversion Report、Receipt、transaction／promotionは[Asset Lifecycle §2.3](../03-authoring/asset-lifecycle.md#gltf-import-adapter-boundary)の`AssetImportJob`、`AssetImportPlanV1`、`SceneImportIRV1`、`AssetConversionReportV1`、`AssetImportReceiptV1`だけが所有する。これらはMaterialsのchannel／BRDF／extension／variant意味を変更しない。
