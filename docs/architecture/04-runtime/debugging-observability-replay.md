...
[Network Transport／Connection](../09-networking/network-transport-connection.md)はconnection epoch、delivery、fault injection、redacted transport telemetryを、[Multiplayer Authority／Replication](../09-networking/multiplayer-authority-replication.md)はsession／authority／baseline／command／correction／rollback／handoff causalityを所有する。本書はそれらのtyped eventをStore／Query／Replay transportへ載せるが、packet arrivalからgameplay causeを推測せず、raw endpoint、credential、full payload／player inputを自動収集しない。Transport fault injectionとauthoritative Replayは別identity／authorization／Receiptである。
...
| AI Production Tooling Host | exact Run／Attempt／step causal projection、bounded read-only Query、Finding、proposal handoff | raw Store access、Runtime mutation、Prompt／Source本文の既定export |
...
[AI Production Orchestration](../03-authoring/ai-production-orchestration.md)のRun、Attempt、Transition、Workflow step、child Operation／Runは、各exact refをcorrelation／parentへ持つbounded event／spanとして投影できる。Debug StoreはRun state、Workflow、Context、ResultまたはEvidenceの正本にならず、欠落eventからstep未実行、Run失敗、Provider refusalまたはProject不変を推測しない。Prompt、Source、Asset、Tool output、Provider response、conversation本文は既定attribute／logへ保存せず、classification、byte count、hash、redaction stateだけを許可する。

OpenTelemetryへのtrace／metric／log exportは`DebugSpanV1`、Counter、Logの明示Adapterであり、canonical Store／Index、Run／Task／Evidence identity、sampling、retention、privacy、causal orderを置換しない。Exporterはregistered stable name／attribute allowlist、redaction、bounded batchを使い、dynamic Prompt／Source／Entity／Credentialをattribute nameまたはvalueへ埋め込まない。Exporter unavailable、drop、retryまたはbackend再順序化をcanonical event success、Run completionまたはEvidence lossへ変換せず、export gapを別typed Diagnosticとして記録する。
...
AI diagnosisを[AI Production Orchestration](../03-authoring/ai-production-orchestration.md)のWorkflowで実行する場合、`AiDebugContextV1`はRun ContextのOwner-typed input、各Debug Operation Taskはchild handle、`DebugFindingV1`またはtyped ProposalはRun Resultのoutput artifactになる。Run／Attempt／Workflow stateをDebug Session、Finding validity、Operation Receipt、Evidence passまたはProject Commitへaliasせず、Run `completed`だけで`validated_cause`、修正済みまたは再現成功を表示しない。external Agent routeも同じAggregate→Query→Causality→Replay→Finding validationのOperation／Receipt chainを短絡できない。
...
support bundleのschema、redaction、size bound、生成operation、failureは本書だけが所有する。`SupportBundleV1`は[Product Plan](../00-product/product-plan.md)と[Product Lifecycle](../00-product/product-lifecycle.md)のdiagnosis→support製品E2E終端を成すUser提出用bundleであり、開発内再現用の`ReproductionBundleV1`とは別概念で相互に代用しない。

Product Lifecycleはsame release／Project／TargetのSupport Bundle取得可能性、User export、support windowをacceptanceへ束縛するだけで、本節のField、redaction、consentまたは生成failureを複写しない。[Product Security](../01-governance/product-security.md)はredacted Bundle componentをvulnerability case／incident Evidenceとして参照できるが、reporter textまたはBundle attachmentをcommand、affected release、severity、disclosure authorityにしない。
...
根拠: official-spec — [OpenTelemetry Specification](https://opentelemetry.io/docs/specs/otel/)はtrace、metric、log等のtelemetry data modelとSDK／export境界を定める。本書のcanonical Store、Run／Task／Evidence、privacy、retentionおよびauthority分離はMiraikanaiのproject-decisionである。
