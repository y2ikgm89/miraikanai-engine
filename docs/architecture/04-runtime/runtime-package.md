...
[AI Production Orchestration](../03-authoring/ai-production-orchestration.md)はAuthoring Toolingだけであり、Runtime Entry／World／UI／headlessのどのPackage branchにもAI Production Orchestrator、Agent Host、Workflow Registry、Run／conversation Store、Provider／Model Credential、MCP Server、Source Worker、Compiler、Signer、write-capable Project Gateway、Prompt templateまたはAuthoring Tool SDKを含めない。package closure、member manifest、dependency scanおよびlaunch image inspectionはこれらのartifact／service roleをexact 0件とし、未参照、dead-code elimination、Credential未設定またはnetwork無効だけをexclusion Evidenceにしない。

deterministic NPC／Gameplay AIはCook済みGameplay contractとして各Domain Ownerから入れられるが、Authoring AIをRuntimeへ移したものではない。将来shipping generative AIを追加する場合はProduct Futureから独立Runtime Capability、Threat／Privacy／network／cost／content safety、offline／service-withdrawal failure、Save／Replay、Target package、positive／negative Fixtureを定義し、本書と対象Runtime Ownerをversionedに更新する。Authoring Provider Profile、Run／Workflow、Model Eval、Consent、CredentialまたはQualificationを流用しない。
...
12. 全Runtime Entry branchのpackage member／dependency／binary／config／environment／credential scanでAuthoring Orchestrator、Agent Host、Workflow Registry、Run／conversation Store、Provider／Model Credential、MCP Server、Source Worker、Compiler、Signer、write-capable Project Gateway、Prompt template、Authoring Tool SDKが0件であり、同じCandidateのBuild／Package Receiptへinspection結果を束縛する。

## 9. 非目的

本書はWorld streaming implementation、Package writer、loader implementation、Task Plan、cache削除またはshipping generative AIを実行しない。各実施はinitial V1 Definition Closure、Qualification要求とProduct Work Packageの条件が満たされた後にだけ開始できる。Authoring AI artifactをRuntimeへ同梱してAI-native、offline supportまたはRuntime AI supportを主張しない。
