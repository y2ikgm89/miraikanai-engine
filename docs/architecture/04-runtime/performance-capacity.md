...
Advanced Light Transportはchannel／Technique／history／representation demandとmeaning fallback、Terrain／Foliageはtile／cluster／artifact demandとdomain fallback、Network Transportはmessage／byte／window／connection pressure disposition、Multiplayerはparticipant／object／snapshot／delta／rollback demandとsession dispositionを所有する。本書は同一Target／fixture／Profile上のCPU／GPU／memory／I/O／network bandwidth／queue measurement、shared envelope、regressionだけを所有し、測定値からTechnique、topology、authority、field drop、rollback windowをsilent変更しない。
...
### 5.4 Authoring AI capacity boundary

[AI Production Orchestration](../03-authoring/ai-production-orchestration.md)はWorkflow step、Model call、external roundtrip、Operation call、retry、child Run、Context／output byte、wall time、exact metering profileに束縛したmodel usage unitおよび観測可能なmonetary budgetを所有する。本書はAI Tooling Host、Provider Adapter、first-party local inference process、Context assembly、Workflow executionが要求するCPU／GPU／RAM／VRAM／I/O／queue capacity、admission、reservation、backpressure、measurement、regressionを所有する。Run budgetの残量、Model token数、Provider costまたはAgent turnをsystem capacityへ変換せず、逆にfree memory、GPU有無またはqueue空きからWorkflow権限を生成しない。

first-party local inference routeはAI Securityのexact Deployment／Model／Loader binding、Toolchain artifact、Target／Device Profileと、本書が判定するcurrent capacity decisionが全て成立する場合だけ開始できる。capacity不足は`queued | rejected | bounded_degradation`のOwner-typed結果へ閉じる。`bounded_degradation`はWorkflow／route Profileが事前に許可したresource envelope内で、Model、Context input、Operation、Targetまたはtyped resultの意味を変えない場合だけ許し、それ以外は`rejected`にする。System memoryへの無制限spill、GameHost／Editor critical reservationの横取り、Runtime frame／simulationの遅延、別GPU／Model／cloudへの暗黙fallbackを許可しない。cloud fallbackはPerformance recoveryではなく、新しいRoute Selection、privacy／region／cost Preview、Context、Authorization、Run、initial Attemptを要求するOrchestration判断である。

external Agentが自Hostでlocal Modelを使う場合、そのCPU／GPU測定はMiraikanai process groupのcapacity Evidenceではない。standard external routeでは外部Provider費用／resourceを`unknown/external-host-owned`とし、0またはbudget-compliantへ補完しない。first-party local inference runtime、managed external Host、shipping Runtime AIのcapacity ProfileとMeasurement Receiptを相互流用しない。

現RepositoryにはAI Tooling capacity request／decision Schema、Profile、Measurement Receipt、first-party local runtimeまたはqualified hardware matrixが存在しない。既存Editor／GameHost provisional budgetからAI workloadのsupport、同時実行数、Model size、Context size、latencyまたはlocal inference可否を推測しない。
