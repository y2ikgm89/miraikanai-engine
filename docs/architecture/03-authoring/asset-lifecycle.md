...
[Game Production Loop §7](game-production-loop.md#7-ai-game-generation-lane)とのmappingはexactに、`domain_pack_reference | user_provided`を`ai_composed_game`、`external_generated`を`ai_generated_external_content`へ割り当てる。`ai_generated_project_source`はAsset `source_kind`ではなく、Native Game ModuleまたはProject Shader Ownerが管理するProject Source promotion経路である。Asset descriptorへ第四kindを追加せず、三lane間の成功、Evidenceまたはfallbackを相互流用しない。
...
一つでも不合格なら旧generation全体を維持する。Textureだけ新、Materialは旧のようなdependency closureの部分混在を禁止する。互換でない変更は`restart_play`を返す。Asset LifecycleはCooked Staging closureとpromotion eligibilityを所有し、Runtime request、activation generation、lease、queue、eviction、retireは[Runtime Asset Lifecycle](../04-runtime/runtime-asset-lifecycle.md)が所有する。
...
外部またはAI生成Assetも通常Importを迂回しない。Asset固有Provenance recordのcontentは本節だけが所有し、共通Field `origin_uri_or_generation_operation_ref`、`acquired_or_generated_at`、`content_hash`、`creator_or_provider`、`terms_snapshot_ref`、`license`、`rights_confirmation_status`、`commercial_review`、`safety`、`content_credential_ref`、`modification_chain[]`を必須にする。originがgeneration operationの場合は`tool_model_lock_ref`、`request_hash`、`input_asset_refs[]`に加え、採用versionのlockとは別に実行証拠の`generation_tool_receipt_ref`を必須とし、外部取得またはUser提供Sourceではこれらgeneration専用Fieldを持たない。Provider outputを直接ProjectまたはCacheへ入れずStagingに置き、このRecordと共通Evidence envelopeを結ぶ。これらのrights／commercial Fieldは[Product Legal／IP Governance](../01-governance/product-legal-ip-governance.md)へ渡すdomain Evidenceであり、法的applicability、非侵害またはRelease Approvalではない。AIが品質または権利を自己承認できない。

[Product Plan](../00-product/product-plan.md)のMVP-A／MVP-Bで必須なのは`ai_composed_game` laneに属する`source_kind=domain_pack_reference | user_provided`だけである。`ai_generated_external_content`のAsset生成ProviderはC2未満のPhase exit前提にせず、`external_generated`が明示要求されたのにqualified Provider、権利Record、safety Receiptのいずれかがない場合は`diagnostic.asset.external-generation-unavailable`を返してProposalを拒否する。Pack reference、user-provided placeholder、別Providerまたは`ai_generated_project_source`へ暗黙fallbackせず、元の要求とProject sourceを変更しない。
